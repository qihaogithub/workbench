...
- 根目录仍有 2 个既有 YAML 临时/诊断产物（`position-after-x-change.yaml`、`position-viewer.yaml`），`check:repo` 会给出 warning；按高频轻量维护规则仅记录、不删除。
...
- 2026-07-02 运行 `corepack pnpm ops:automation stale-issues --json`，当前扫描 9 个进行中计划，`flagged=1`；命中 `docs/plans/进行中/创作端编辑与协同.md` 的 `completed-in-progress` 信号，但正文仍保留真实浏览器复验与手工验收待办，本轮仅调整状态措辞，不归档、不合并。
...
- 2026-07-03 每日完整回归中，`corepack pnpm check:automation`、`corepack pnpm check:all` 均通过；仅发现 E2E 前置服务缺失，`3200/3202/3300` 未监听，因此按环境阻塞跳过 `corepack pnpm test:e2e:core-flow`，不创建新的业务缺陷或测试缺口文档。
...
- 2026-07-01：`corepack pnpm check:automation` 通过；`corepack pnpm ops:automation report --json` 显示 13 个 active 入口；本轮修复计划文档相对路径漂移后，`corepack pnpm check:repo` 通过并回到当时既有根目录临时/诊断产物 warning 基线。
...
- 2026-07-02：本轮高频轻量检查中，`corepack pnpm check:automation` 通过，`corepack pnpm ops:automation report --json` 仍为 13 个 active；再次修复 `diagnostics:*` 根脚本路径漂移后，`corepack pnpm check:repo` 回到仅 2 条既有 YAML 临时产物 warning，未新增业务缺陷或测试缺口记录。
- 2026-07-02：本轮问题清理中，`corepack pnpm check:automation` 通过；`corepack pnpm ops:automation stale-issues --json` 返回 `scanned=9`、`flagged=1`，唯一候选为 `docs/plans/进行中/创作端编辑与协同.md` 的 `completed-in-progress`。复核后确认该文档仍有真实浏览器复验与导出包手工验收待办，因此仅将条目状态统一改为“暂不归档”，未移动到 `docs/plans/已完成/`。
- 2026-07-03：`corepack pnpm check:automation` 通过，`corepack pnpm ops:automation report --json` 仍为 13 个 active，`corepack pnpm check:all` 全量通过。端口与 HTTP 探测显示仅 `3201` 可用，`3200/3202/3300` 均拒绝连接；本轮据此按 `E2E 服务前置缺失` 分类跳过 `corepack pnpm test:e2e:core-flow`，未新增 `docs/plans/进行中/` 问题记录。
