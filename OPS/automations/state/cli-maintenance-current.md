...
`commands --json`、`packages/project-cli/src/index.ts` 的 `register(...)` 列表与 `packages/project-cli/src/cli-all-commands.test.ts` 末尾的未覆盖守卫继续保持一致；本轮未发现新的注册命令或测试覆盖缺口。此前完成的 runtime contract 校验 CLI 对齐仍保持有效：`project validate-runtime`、`page validate-runtime` 已注册，CLI 测试与能力清单保持同步。
...
本轮发现并修复的是低风险文档漂移：`page switch-runtime` 已在 CLI、全命令测试和 `project-core.switchPageRuntime` 中生效，但 `05_CLI能力自动化清单.md` 未登记该能力。当前事实文档已补齐，避免下一轮自动任务误报“已注册命令漏登记”。
