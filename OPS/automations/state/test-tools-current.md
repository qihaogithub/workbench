...
- `check:repo` 仍可能因历史归档迁移暴露 Markdown 坏链；优先修复 `docs/项目文档/` 内失效的相对引用。当前基线仅剩 2 条根目录 YAML 临时/诊断产物 warning（`position-after-x-change.yaml`、`position-viewer.yaml`），不自动删除。
...
- 每日完整回归在运行 E2E 前必须先确认 `3200/3202/3300` 服务端口；若 `author-site` 基础入口 `3200` 未监听，应直接记录为环境阻塞，不强行运行 `corepack pnpm test:e2e:core-flow`。
...
- 2026-07-02：本轮高频轻量检查中，`corepack pnpm check:automation` 通过，`corepack pnpm ops:automation report --json` 仍为 13 个 active；再次修复根 `package.json` 的 `diagnostics:*` 路径漂移后，`corepack pnpm check:repo` 回到仅 2 条根目录 YAML 临时产物 warning。
- 2026-07-03：`corepack pnpm check:automation` 通过，`corepack pnpm ops:automation report --json` 仍为 13 个 active；`corepack pnpm check:all` 全量通过。端口检查与 HTTP 探测显示仅 `3201` 监听且返回 404 JSON，`3200/3202/3300` 均未监听，因此本轮按 E2E 服务前置缺失跳过 `corepack pnpm test:e2e:core-flow`，未创建新的测试工具或业务缺陷记录。
