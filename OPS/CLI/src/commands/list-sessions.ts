...
export async function listSessions(
  ...
  const spinner = createSpinner('正在查询会话列表...');
  ...
  try {
    ...
    sessions.forEach((session, index) => {
      ...
      const status = getSessionColor(session.status)(session.status.padEnd(11));
      ...
      console.log(`${idx} | ${id} | ${status} | ${backend} | ${msgCount} | ${lastActivity}`);
    });
