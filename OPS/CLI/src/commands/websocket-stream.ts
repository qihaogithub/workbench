...
import { createSpinner, showSuccess, showError, formatDuration } from '../utils.js';
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    ws.on('message', (data: WebSocket.Data) => {
      try {
        ...
        switch (event.type) {
          ...
          case 'thought':
            if (event.content) {
              console.log(chalk.dim(event.content));
            }
            break;

          case 'tool_call':
            console.log(chalk.cyan(`\n[工具调用] ${event.title || event.toolCallId} (${event.kind})`));
            break;

          case 'tool_call_update':
            console.log(chalk.gray(`[工具状态] ${event.toolCallId}: ${event.toolCallStatus}`));
            break;
          ...
          case 'finish':
            ...
            console.log(chalk.gray(`内容长度: ${accumulatedContent.length} 字符`));
          ...
          case 'error':
            ...
            if (event.error?.message?.includes('No active session')) {
              ...
              console.log(chalk.yellow('  - 检查 agent-service 日志获取详细信息'));
              console.log(chalk.yellow('  - 查看日志: 在 agent-service 目录运行 pnpm dev:agent'));
            } else {
              console.log(chalk.yellow(`  - 错误信息: ${event.error?.message}`));
            }
    ...
    ws.on('close', (code) => {
      if (!isComplete) {
        ...
        console.log(chalk.gray(`已接收内容: ${accumulatedContent.length} 字符`));
    ...
    ws.on('error', (error) => {
      ...
      console.log(chalk.gray(`  - 使用新会话重试: ops-cli stream "test-${Date.now()}" "测试消息"`));
      console.log('');
