...
| 变量 | 示例值 | 说明 |
|:-----|:-------|:-----|
| `NEXT_PUBLIC_WEB_URL` | `http://localhost:3200` | web 创作端地址，用于生成 iframe URL 和调用配置 API |
...
web 创作端使用 Next.js 中间件处理 CORS，针对 API 路由和嵌入路由设置响应头：

- 仅对 `/api/` 和 `/embed/` 路由添加 CORS 头
...
| `pnpm dev:all` | 同时启动 web、agent-service、viewer-site 三个服务 |
| `pnpm dev` | 启动 web + agent-service（不含 viewer-site） |
