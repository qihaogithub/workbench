...
1. 主 Agent 调用 `delegateTask`，传入任务说明和可选上下文。
2. `delegateTask` 把请求交给 `PiAgentBackend`，由 backend 创建独立的 `AgentHarness`、执行环境和内存会话。
3. 子 Agent 使用与主 Agent 相同的 Workbench 工具执行任务，但没有委派工具。
4. 子 Agent 结束后，backend 提取文本结果，并把本次新增的文件变更写回父会话的 `files` 列表。
5. 前端仍按普通工具调用展示 `delegateTask`，不需要理解子 Agent 的内部事件。
...
子 Agent 是一次性资源。每次委派都会创建独立 harness 和内存 session，执行完成、超时、取消或失败后立即清理。
...
主请求取消或 backend 销毁时，所有仍在运行的子 Agent 会被中止，避免后台任务继续写入工作空间。
