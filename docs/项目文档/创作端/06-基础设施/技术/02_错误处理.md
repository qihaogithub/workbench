...
AI 对话和使用端问答会额外经过共享错误归一化层。它把连接失败、超时、鉴权失败、额度或频率限制、服务端异常、请求取消和未知异常映射成普通用户可理解的中文提示。

这层归一化只改变对用户展示的 `message`，不吞掉技术信息。前端诊断事件、agent-service 日志和服务端错误对象仍保留原始错误文本，便于开发者排查。
...
**文件位置**：`@workbench/shared`（`packages/shared/src/index.ts`）
