...
正式环境的 Compose 配置会限制每个业务容器的 CPU、内存和进程数，避免单个服务或浏览器进程把宿主机资源耗尽：
...
| `APP_DATA_DIR`                       | `/opt/opencode-workbench/data`    | 宿主机持久数据目录，绑定到容器 `/app/data` |
...
`docker-compose.yml` 使用 `${APP_DATA_DIR:-/opt/opencode-workbench/data}:/app/data` 绑定宿主机目录。这个目录是生产数据源，至少包含：
...
- 同步阶段会排除历史 `packages/web/`、历史 `packages/snapshot-service/`、根 `.opencode/`、包内 `.next/`、包内 `data/`、包内 `.opencode/` 和各类依赖/构建产物，避免把本地缓存、历史工作区或测试数据传入正式机。
...
| agent-service 重启后模型供应商回退 | 运行时内存配置尚未恢复或内部同步失败 | 检查 author-site 日志中的 `BackendProviders Sync`，确认 `INTERNAL_API_TOKEN` 一致 |
