...
- 先停止本项目本地 Compose 中的 `author-site` 和 3200 端口的现有进程，但保留 `agent-service`、`screenshot-service`、`viewer-site` 和本地 `data/`。
...
- 只有 production build 成功才补齐 standalone 产物的 `public`/静态资源并启动 standalone server；构建失败时直接终止，不回退启动上一次产物。
- 默认使用根目录 `data/` 和 `localhost:3201/3202`；依赖服务不可用时输出警告，但不阻断纯页面验收。

仅验证构建而不启动页面时，使用 `corepack pnpm preview:local -- --build-only`。查看将执行的操作但不停服务、不构建时，使用 `--dry-run`。
...
不要把管理后台配置只依赖 Docker named volume。named volume 会受 compose project 名称、`down -v` 和平台清理策略影响。生产环境应固定 `APP_DATA_DIR`；部署脚本默认拒绝缺失的目录，只有首次部署显式设置 `ALLOW_CREATE_APP_DATA_DIR=true` 才创建空目录。备份 SQLite 时应停服务，或同时保留 `users.db`、`users.db-wal` 和 `users.db-shm`。
...
- 默认设置 `DEPLOY_BUILD_MODE=local`，在本机按 `DEPLOY_IMAGE_PLATFORM=linux/amd64` 构建 Docker 镜像，导出为压缩归档上传到服务器；服务器只执行 `docker load` 和 `docker compose up --no-build`，避免正式机承担 Next.js、pnpm install 或 apt 依赖安装的构建压力。
- 默认设置 `COMPOSE_PARALLEL_LIMIT=1`，限制本地或兜底远程构建并发。
- 在任何同步或构建前运行 Workspace Authority 部署前检查：本地静态门禁拒绝旧 live Workspace 写入旁路，Compose 检查确认 `agent-service`、`author-site` 和 `screenshot-service` 共用 `/app/data`，远程只读扫描正式 `APP_DATA_DIR`。
