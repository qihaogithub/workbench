...
`project-core` 通过 `WorkspaceMutationPort` 接口与 Authority 通信。所有写入型事务方法在实际落盘前调用 `getWorkspaceAuthorityPort()` 获取端口；如果端口存在且目标工作区为 live scope，写入必须经过 Authority mutation 提交并获得 receipt。端口未配置时，live Workspace 写入直接返回 `WORKSPACE_AUTHORITY_REQUIRED`，不降级到裸文件写入。
...
## OPS CLI Workspace Authority 命令

OPS CLI（`OPS/CLI/`）提供 5 个面向运维诊断的 workspace-authority 命令，用于排查 Authority 状态和修复 drift：

| 命令                                                              | 说明                                                                              |
| :---------------------------------------------------------------- | :-------------------------------------------------------------------------------- |
| `workspace-authority-status <projectId> <workspaceId>`            | 只读查询 Authority 状态：ready、revision、rootHash、queue、lease、prepared、drift |
| `workspace-authority-preflight <projectId> <workspaceId>`         | 只读检查 Authority 是否满足关键动作前置条件，输出 passed/issues                   |
| `workspace-authority-bootstrap <projectId> <workspaceId>`         | 默认 dry-run 检查 bootstrap 状态；加 `--apply` 才创建 Authority state             |
| `workspace-authority-reconcile-adopt <projectId> <workspaceId>`   | 默认 dry-run 检查 external drift 接纳；加 `--apply` 才执行                        |
| `workspace-authority-reconcile-restore <projectId> <workspaceId>` | 默认 dry-run 检查 committed backup 恢复；加 `--apply` 才执行                      |

这些命令是只读诊断和受控修复入口，不绕过 Authority 事务协议。reconcile 命令默认 dry-run 的设计与 agent-service 的 REST reconcile API 保持一致。
