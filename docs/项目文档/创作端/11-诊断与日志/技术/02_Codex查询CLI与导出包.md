...
当 SQLite 不可用、缺失或查询不到可用数据时，CLI 会扫描 `data/editor-diagnostics/*.jsonl`。兜底路径必须显式标记，不能静默返回空事件。
...
导出包只保存摘要和索引。完整源码、完整 prompt、完整模型回复、完整工具结果、密钥、token、cookie、上传文件正文和数据库快照都不得进入导出包。
