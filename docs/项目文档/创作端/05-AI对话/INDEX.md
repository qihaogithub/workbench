...
页面工具已增加跨项目一等能力：Agent 通过服务端绑定身份列出可用源页面、发起复制/引用、查询任务、提交显式冲突决策和撤销引用；工具按 Authority receipt 驱动工作区刷新，不模拟剪贴板。领域契约与权限边界见[跨项目页面转移实现设计](../03-项目管理/技术/14_跨项目页面转移.md)。
...
| 文档                                        | 说明                                                          |
| :------------------------------------------ | :------------------------------------------------------------ |
| [AI对话\_需求文档.md](./AI对话_需求文档.md) | AI 对话、流式响应、点阵等待态、回复期间消息排队与超时恢复、工具调用、自动标题与 30 天保留的 Popover 对话历史（右上角轻量新建按钮）、消息编辑与重发、模型偏好、统一加号附件菜单、图片直传、本地文件附件读取与计划审批需求、引用项目（跨项目读取）、图像生成与抠图，以及管理员只读查看边界 |
| [项目清单\_需求文档.md](./项目清单_需求文档.md) | 项目清单的资源浏览、搜索筛选、简介维护、治理文档分组、来源异常和生成状态展示需求 |

### 技术文档

| 文档                                                | 说明                                                                                                                                           |
| :-------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------- |
| [01\_对话组件设计.md](./技术/01_对话组件设计.md)    | AIChat/HistoryDialog 组件设计、编辑页进入时新建 AI 对话、共享加号附件菜单、用户/对话级附件授权与账本关联、图片预览去重展示、DotMatrix 等待态、首轮自动标题、模型偏好、消息队列、会话 busy 防线、审批等待态与历史工具卡终态、可视化图层列表元素引用入口 |
...
| [04_用户模型配置.md](./技术/04_用户模型配置.md)     | 用户自定义 OpenAI 兼容 API、API Key 加密存储、个人 provider 优先级、全局配置保存后的活跃 Session 刷新与会话级配置同步                         |
...
| [08_用户级外部工具授权.md](./技术/08_用户级外部工具授权.md) | Figma MCP 与钉钉 dws 的用户级授权、加密存储、session 注入、固定回调跨环境 handoff、工具范围和写操作确认                                                                  |
...
| [11_对话账本与恢复.md](./技术/11_对话账本与恢复.md) | SQLite 权威对话账本、单条幂等 ACK 与 IndexedDB 有界重试、附件事务与删除治理、基于 revision 的编辑/重发/重新生成、Agent 版本化上下文摘要与重建、run 终态、owner 隔离、有界 run artifact、30 天硬删除、结构化脱敏轨迹、管理员只读查询、备份轮换、可靠性看板与默认关闭的脱敏分析投影 |
| [项目清单与语义索引](./技术/12_项目资源清单与语义索引.md) | 项目清单的确定性目录、自动保存与 CAS 三方合并、generation lease/异步语义生成、Authority 白名单 evidence、agent-service stateless invoker、轮询隔离、权限过滤、统一 L3 投影和 `searchProjectInventory` |
