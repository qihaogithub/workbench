...
- 图层列表按视觉层级展示对象，并优先使用节点 `name` 作为管理标签；没有 `name` 时才回退到画布文本或类型。它不再常驻在侧栏，而是作为选区工具栏“图层”或“更多 → 图层管理”的锚定气泡内容；搜索筛选、重命名、拖放排序、状态徽标和右键命令保持不变，且不写入 `SketchSceneDocument`。
- 属性面板继续提供通用、内容、位置、布局/排列、线条/连接器、路径、外观、文本、图片、绑定和导出区域，并保留所有既有字段校验、连续输入 history 和多选共同样式规则。它作为选区工具栏“属性”或“更多 → 属性与导出”的锚定气泡内容；单选工具栏提供填充、描边、文本、复制样式、属性、图层和更多入口，多选工具栏提供对齐、分布、成组、复制、删除和更多入口。填充、描边点击后展开调色气泡；同一时间只显示一个菜单，选区取消、Escape、点击外部或关闭按钮均会退出。工具条优先定位在选区上方，顶部空间不足时放在下方，内联文本编辑、拖拽、框选或绘制期间隐藏。样式复制/粘贴使用独立样式剪贴板，按目标对象支持的字段应用外观字段，不把不适用字段强行写入目标对象。
...
质量护栏以 `@workbench/sketch-react` 单测和 sketch playground E2E 双层维护：单测覆盖上下文工具条、锚定气泡菜单、键盘微调、Enter 提交、连接器绑定和端点跟随等行为；playground E2E 覆盖空白画布、底部工具栏、选区编辑、图层气泡、缩放和内联文本编辑。性能基准属于自动化测试与开发诊断，不再作为 Playground 的常驻用户界面。
