...
> **状态**：已完成 | **日期**：2026-06-04 | **完成日期**：2026-06-26 | **前置**：`预览区画布模式Puppeteer截图方案.md`（共享 Puppeteer 基础设施）
...
> **2026-06-26 完成记录**：补齐控制台缓冲、控制台工具、author 侧控制台批量转发测试；截图工具、截图路由、系统提示词和相关类型检查验证通过。真实外部 LLM 的多模态端到端调用未在本地执行，作为剩余人工验收风险记录。
...
| `screenshot-tool.ts`  | `packages/agent-service/src/backends/pi-tools/` | `captureScreenshot` 工具定义         | ✅   |
...
| 文件                        | 位置                                           | 说明                              | 状态 |
| --------------------------- | ---------------------------------------------- | --------------------------------- | ---- |
| `console-buffer.test.ts`    | `packages/agent-service/tests/unit/`           | 控制台缓冲过滤、裁剪和清理测试    | ✅   |
| `console-tool.test.ts`      | `packages/agent-service/tests/unit/`           | `getConsoleLogs` 空结果和过滤测试 | ✅   |
| `screenshot-tool.test.ts`   | `packages/agent-service/tests/unit/`           | `captureScreenshot` 图片返回测试  | ✅   |
| `useConsoleBuffer.test.tsx` | `packages/author-site/src/components/demo/`    | 控制台日志 100ms 批量转发测试     | ✅   |
...
- `getConsoleLogs` 已随 Pi Agent 工具注册测试覆盖，并补充 `consoleBuffer`、`console-tool`、`useConsoleBuffer` 的细粒度测试
...
### Phase 3：System Prompt 与集成测试 ✅ 已完成

1. ✅ 更新静态 system prompt，追加 `captureScreenshot` 使用指引
2. ✅ 控制台链路自动化验证：`useConsoleBuffer` 100ms 批量转发、`consoleBuffer` 会话缓冲、`getConsoleLogs` 读取与格式化
3. ✅ 截图链路自动化验证：`captureScreenshot` 调用截图服务、传递 `fullPage`、返回 PNG `ImageContent`
4. ✅ 多模态消费前置验证：工具返回 `ImageContent`，Pi Agent 对多模态图片输入已有 `base64` 适配；真实外部 LLM 端到端调用保留为人工验收项

### Phase 4：收尾验证 ✅ 已完成

1. ✅ `pnpm --filter @opencode-workbench/agent-service test -- console-buffer.test.ts console-tool.test.ts screenshot-tool.test.ts`
2. ✅ `pnpm --filter @opencode-workbench/screenshot-service test -- screenshots-routes.test.ts`
3. ✅ `pnpm --filter @opencode-workbench/author-site test -- useConsoleBuffer.test.tsx useScreenshotGeneration.test.tsx system-prompt.test.ts`
4. ✅ `pnpm --filter @opencode-workbench/agent-service typecheck`
5. ✅ `pnpm --filter @opencode-workbench/screenshot-service typecheck`
6. ✅ `pnpm --filter @opencode-workbench/author-site typecheck`
7. ⚠️ `pnpm --filter @opencode-workbench/agent-service test -- pi-agent.test.ts` 当前受未提交的 `knowledgeReport` 工具改动影响失败，失败点是工具数量和只读能力断言，不属于本方案变更范围
...
| 真实 LLM 多模态端到端未本地执行       | 无法证明所有外部模型都能消费工具返回图片 | 已用自动化验证 `ImageContent` 返回结构；上线或人工验收时用真实模型补一次对话闭环 |
