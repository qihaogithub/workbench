...
- 2026-06-28：复查首期完成情况后，补充 `APP_ACTION -> viewer shell` 的纯函数验证。viewer 动作解释逻辑已抽出为 `viewer-app-graph-runtime`，测试覆盖按当前 `pageId` 映射 `routeKey`、匹配 `from + event`、筛选 route params、写入运行时 state 并返回目标页面。
...
- `PATH="$HOME/.nvm/versions/node/v20.19.0/bin:$PATH" E2E_BASE_URL=http://localhost:3210 E2E_USER=e2e_ai_native_0626 E2E_PASSWORD=e2e-pass-0626 corepack pnpm exec playwright test --config test/创作端E2E回归测试/playwright.config.ts`
- `corepack pnpm --filter @opencode-workbench/author-site test -- --runTestsByPath src/lib/__tests__/viewer-app-graph-runtime.test.ts`
- `corepack pnpm --filter @opencode-workbench/author-site typecheck`

端到端结果：1 个 Playwright 用例通过，覆盖项目创建、登录、创建页面、通过编辑页 API 写入页面代码与 Schema、合并保存版本、返回首页删除项目。

补充验证结果：`APP_ACTION -> viewer shell` 的运行时解释链路已由单元测试覆盖；该测试不依赖 Next 路由和 DOM，专注验证应用图执行语义。
...
- `routeKey` 在共享类型中保持为可选字段，但工作区读写、页面创建、页面复制和旧项目读取会自动补齐并固化，作为逐步迁移策略。
- 创作端应用图固定读写 `workspace/app.graph.json`；本地 scaffold manifest 通过 `appGraph` 指向 `src/app.graph.json`。
- SDK action 首期同时提供 `trigger`、`PageAction`、`useAppState()` 和 `useRouteParams()`。
- 画布首期不展示应用图连线，仅在 viewer 运行时执行应用图。
- 工程交接包首期已纳入基础范围：发布产物和 scaffold 输出都会携带 `routeKey` 与应用图。
