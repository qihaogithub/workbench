...
高优先级和中优先级差异项已实施完成（#1-#4, #6, #11, #12），低优先级项（#5, #7, #10）暂不实施，#8 grep/find 待评估 ROI。
...
| 优先级 | 差异项 | 状态 | 理由 |
|--------|--------|------|------|
| **高** | #2 readFile 输出截断 | ✅ 已实施 | 大文件读取直接导致 token 爆炸，影响对话质量 |
| **高** | #4 bash 输出截断 | ✅ 已实施 | 大输出命令直接报错而非优雅降级 |
| **中** | #1 readFile offset/limit | ✅ 已实施 | 与 #2 配合，提供大文件分页读取能力 |
| **中** | #3 bash timeout 参数 | ✅ 已实施 | 长命令被 30s 硬限制切断 |
| **中** | #6 editFile prepareArguments | ✅ 已实施 | 防止部分模型的 JSON 字符串 edits 导致错误 |
| **中** | #8 grep/find 探索工具 | ⏳ 待评估 ROI | 减少模型通过 bash 搜索的间接路径 |
| **中** | #11 AbortSignal 接入 | ✅ 已实施 | 中止操作时避免无用的文件/命令执行 |
| **中** | #12 bash onUpdate 流式 | ✅ 已实施 | 长命令实时反馈 |
| **低** | #5 promptGuidelines | ⏭️ 暂不实施 | 当前 system-prompt.md 手动维护功能等价 |
| **低** | #7 diff/patch 输出 | ⏭️ 暂不实施 | 无 TUI 渲染需求 |
| **低** | #10 工具名称 | ⏭️ 暂不实施 | 影响微小 |
...
### 第一阶段（短期高收益）— ✅ 已完成
- ✅ readFile 合并 offset/limit + 添加 truncateHead 截断（新增 `truncate.ts` 共享截断工具）
- ✅ bash 添加 truncateTail 截断 + timeout 参数（从 `exec` 切换到 `spawn`，支持流式和取消）
- ✅ editFile 添加 prepareArguments（JSON 字符串修复）
- ✅ 合并 readFileWithLines 到 readFile（移除独立工具，`readFileWithLines` 已删除）

### 第二阶段（中期增强）— ✅ 已完成
- ✅ 为 bash 添加 AbortSignal 检查 + onUpdate 流式更新
- ⏳ 评估添加 grep/find 工具的 ROI（待定，当前 bash 白名单已含 grep/find）

### 第三阶段（长期优化）— ⏭️ 暂不实施
- ⏭️ 评估 promptGuidelines 自动注入机制
- ⏭️ 评估 editFile diff/patch 输出对 UI 的价值
...
| 文件 | 角色 |
|------|------|
| `packages/agent-service/src/backends/pi-tools/edit-file-tool.ts` | editFile 工具实现（已添加 prepareArguments） |
| `packages/agent-service/src/backends/pi-tools/file-tools.ts` | readFile/writeFile/listFiles 工具实现（readFile 已添加 offset/limit + 截断） |
| `packages/agent-service/src/backends/pi-tools/bash-tool.ts` | bash 工具实现（已切换到 spawn，添加 timeout/截断/signal/onUpdate） |
| `packages/agent-service/src/backends/pi-tools/truncate.ts` | 共享截断工具（truncateHead/truncateTail，新增） |
| `packages/agent-service/src/backends/pi-tools/index.ts` | 工具注册入口（已移除 readFileWithLines，版本升至 22） |
| `packages/agent-service/src/backends/pi-agent.ts` | harness 初始化 + system prompt 构建 |
| `packages/author-site/src/lib/agent/prompts/system-prompt.md` | System Prompt（已移除 readFileWithLines 引用） |
