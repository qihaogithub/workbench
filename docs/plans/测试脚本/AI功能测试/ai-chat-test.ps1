...
param(
    ...
    [string]$message = "你好，请帮我生成一个简单的按钮组件",
...
try {
    ...
    if ($inputMatch -and $inputMatch.Matches.Count -gt 0) {
        ...
        if ($hasTextarea -eq "true") {
            ...
            Log "Using textarea selector" "DEBUG"
        } else {
    ...
    $inputValue = playwright-cli --raw eval "(function() { var t = document.querySelector('textarea'); if (t) return t.value; var p = document.querySelector('[placeholder*=\u8f93\u5165]'); if (p) return p.value; return ''; })()" 2>$null
    ...
    while ($true) {
        ...
        if ($elapsed - $lastStreamCheck -ge 10) {
            ...
            $pageState = playwright-cli --raw eval "JSON.stringify({streaming: document.querySelectorAll('[class*=streaming], [class*=loading]').length, assistantMsgs: document.querySelectorAll('[data-role=assistant], .assistant').length, userMsgs: document.querySelectorAll('[data-role=user], .user').length, bodyTextLength: document.body.innerText.length})" 2>$null
            Log "[$([int]$elapsed)s] Page state: $pageState" "DEBUG"
        }

        $pageText = playwright-cli --raw eval "document.body.innerText" 2>$null
        if ($pageText -match "\u62b1\u6b49\uff0c\u6211\u6ca1\u6709\u6536\u5230\u6709\u6548\u7684\u56de\u590d") {
        ...
        if ($isStreaming -eq "false" -and $elapsed -gt 5) {
            $aiReply = playwright-cli --raw eval "(function() { var msgs = document.querySelectorAll('[data-role=assistant], .assistant, [class*=assistant]'); if (msgs.length > 0) { var lastMsg = msgs[msgs.length - 1]; return lastMsg.innerText.substring(0, 300); } return ''; })()" 2>$null
    ...
    $finalState = playwright-cli --raw eval "JSON.stringify({url: window.location.href, title: document.title, bodyTextLength: document.body.innerText.length, bodyTextPreview: document.body.innerText.substring(0, 500), streamingElements: document.querySelectorAll('[class*=streaming], [class*=loading]').length, assistantMessages: document.querySelectorAll('[data-role=assistant], .assistant, [class*=assistant]').length, userMessages: document.querySelectorAll('[data-role=user], .user, [class*=user]').length})" 2>$null
