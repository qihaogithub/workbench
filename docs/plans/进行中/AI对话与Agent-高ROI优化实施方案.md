...
Pi Agent 已具备上下文预压缩、一次上下文溢出恢复、超时控制、权限确认和基础重试能力。当前仍有一组高收益问题：工具执行后发生短暂错误可能整轮重放；每轮无差别带入历史附件和大量工具描述；高频运行事件同步写磁盘；重连或编辑时需要从客户端逐条回放完整历史；取消和前端流式体验缺少完整的状态与结果闭环。
...
项目附件清单采用会话级短 TTL 缓存，并通过附件版本或内容哈希失效；本轮上传绕过缓存，保证即时可见。模型可通过稳定 attachmentId 请求内容，禁止按文件名猜测。
