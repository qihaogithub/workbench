...
- [x] 替换当前项目中已确认不可用的图标导入。
- [x] 修复 `RUNTIME_ERROR` 的内部记录链路，确保错误能进入 AI 上下文。
- [x] 预览区不显示技术错误，改为友好占位或保留上一张成功截图。
- [x] 为截图服务记录运行时错误原因，避免只产出纯白截图。
...
- [x] 新增预览依赖策略模块。
- [x] `compiler.ts` 不再为第三方依赖生成未固定版本 CDN URL。
- [x] `/api/compile`、viewer、截图、发布、嵌入共用同一依赖解析入口。
- [x] 未登记依赖返回结构化错误给 AI，不询问用户。
...
- [x] 提供 `@preview/sdk` 或等价虚拟模块。
- [x] 首批实现 `Icon`、基础 UI、图片、格式化、倒计时、进度条等基础能力。
- [x] 预置增强能力：动画、图表、庆祝效果、Lottie、媒体可视化、轮播。
- [x] 明确暂不开放 3D、地图、富文本编辑/渲染、日历能力。
- [x] 编译器将 `@preview/sdk` 映射到受控运行时实现。
- [x] AI prompt 改为要求使用 SDK 能力，不直接使用底层 npm 包。
...
- [x] 编译前增加页面运行时契约校验。
- [x] 将校验错误、编译错误、运行错误统一成 AI 可读的结构化上下文。
- [x] 增加“一次失败自动修复一次”的机制，避免用户手动催促 AI 排查。
- [x] 对连续失败设置安全上限，避免无限循环。
...
- 新增预览依赖策略与 `@preview/sdk` 虚拟模块，固定 React、React DOM、lucide-react、framer-motion 等受控依赖版本。
- 编译器在 Sucrase 转换前执行页面运行时契约校验，拦截未知 npm import、相对源码 import、无效 lucide named import 和明显空渲染风险。
- iframe 运行时将错误写入隐藏结构化标记并回传父页面，用户侧仅看到“预览生成中”或“正在修复预览”等友好状态。
- 编辑页和画布预览将编译错误、运行错误注入 AI 当前视图上下文，并对同一页面设置一次自动修复触发上限，避免连续失败无限循环。
- 截图服务在截图前读取运行时错误标记，返回 `RUNTIME_ERROR`，不再把运行失败页面写入截图缓存。
- 修复 `proj_1782286923644` 及其当前 workspace 副本中的 `external-guide_k7m2`、`age-confirmation_q8n5` 图标导入，改用 `@preview/sdk` 语义图标。
- 更新 AI 页面生成提示词，要求优先使用 `@preview/sdk`，不再通过注释引入白名单外 npm 包。

验证结果：

- `pnpm --filter @opencode-workbench/author-site test -- preview-runtime-policy.test.ts` 通过。
- `pnpm --filter @opencode-workbench/screenshot-service test -- browser-pool.test.ts` 通过。
- `pnpm check:author` 通过。
- `pnpm check:screenshot` 通过。
- `pnpm check:viewer` 通过。
- `pnpm check:all` 通过。

剩余风险：

- `@preview/sdk` 当前是面向页面运行时的首版轻量实现，复杂图表、Lottie 真实播放和高级动画后续可按产品需求继续增强，但不会开放任意第三方依赖。
