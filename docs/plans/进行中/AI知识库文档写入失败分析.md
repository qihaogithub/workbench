...
- `pnpm check:agent`：394 个测试全通过（含 2 个新增 manifest 同步测试、4 个更新的权限测试）
...
- 测试文件：
  ...
  - `packages/agent-service/tests/unit/file-tools-live-workspace.test.ts` — 新增 manifest 同步和覆盖写入不触发同步 2 个测试
