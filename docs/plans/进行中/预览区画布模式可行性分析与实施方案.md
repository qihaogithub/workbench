...
> 版本：v6.1 | 更新日期：2026-06-03 | 方案：图片（Puppeteer 截图）+ 单 Iframe 混合画布 | 已校正
...
export interface PreviewCanvasProps {
  ...
  /** 点击画布空白区域回调，关闭配置编辑 */
  onCanvasClick?: () => void;
  /** 预加载的截图 URL 映射：pageId → URL */
  snapshots?: Record<string, string>;
  /** 截图版本号（代码变更时递增），用于触发 <img> 标签 URL 刷新 */
...
export function PreviewCanvas({
  ...
  // 当外部 state 的 pages 为空时，自动计算初始布局
  ...
  const anyMissing = pages.some((p) => !snapshots[p.id]);
...
const loadSnapshots = useCallback(async () => {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_AGENT_SERVICE_URL || ""}/api/snapshots/list/${demoId}`,
    );
    const data = await res.json();
    if (data.success) {
...
**代码/配置变更时后台生成截图**（关键：截图在编辑过程中就已生成好）：
```typescript
// 在 applyDemoSnapshot 或代码保存时触发
...
// 当 snapshotVersion 变化时（表示代码/配置变更），后台生成截图
useEffect(() => {
...
画布模式下，`editable=false`，不支持拖拽调整页面位置，但**支持悬浮「修改配置」→ 点击切换为 iframe 实时预览**。同样通过 `GET /api/snapshots/list/:projectId` 加载已有截图。双击行为与创作端一致：无切单页逻辑。
...
| 截图生成延迟导致画布打开无图 | 高 | 中 | 编辑时即后台生成截图；画布打开缺失则占位 + 后台补充 |
| `<img>` 跨域问题 | 低 | 中 | `<img>` 直接访问 agent-service，CORS 不限制图片 |
...
| `packages/shared/src/demo/index.ts` | 导出新组件和新类型 |
| `packages/author-site/components/demo/index.ts` | 重导出新组件 |
