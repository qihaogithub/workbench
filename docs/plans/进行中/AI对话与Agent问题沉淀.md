...
- [ ] P0：`backend-agent.ts` 中增加 LLM API 自动重试（指数退避，针对 429/5xx/超时）
- [ ] P0：`backend-agent.ts` 中区分 rate-limit 错误，使用 `RATE_LIMIT_EXCEEDED` 错误码
- [ ] P0：`websocket.ts` 中 rate-limit 错误后销毁 Agent，下次消息创建全新 Agent
- [ ] P1：前端 `normalizeAiError` 根据 `error.code` 分类，不再依赖文本匹配
- [ ] P2：评估对话历史 token 截断策略
