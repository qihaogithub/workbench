...
- **`buildRuntimeConversionPrompt` 简化**：从约 45 行内联规则简化为仅携带页面元信息 + 触发 skill 的指令
...
4. 附带收益：将 system prompt 瘦身到约 310-330 行核心内容（-46%~-49%），降低每轮对话的 prompt token 成本
...
| 涉及包 | 涉及文件 | 改动类型 |
|--------|----------|----------|
| `@workbench/agent-service` | `src/preinstalled-skills/` | 新增 9 个内置 SKILL.md 文件 |
...
| `@workbench/author-site` | `src/app/demo/[id]/edit/page.tsx` | 简化 `buildRuntimeConversionPrompt`：仅保留页面元信息 + 触发 skill 指令，完整规则移入 `page-runtime-conversion` skill |
...
| 权限确认 | ~10 | 安全基线 |
| 需求确认 | ~12 | UI 规范 |
| Tier 2 最小可行约束（4 个 skill） | ~20 | 结构正确性保底，详见各 skill 说明 |
| on-demand skill 引用指令（9 个 skill） | ~27 | 指向 `readPreinstalledSkill` 的简短触发提示 |
| memory.md 核心入口 | ~5 | 告知 agent 首消息自动注入 memory.md |

### 二、迁移为内置 Skill（约 310 行）

新增 9 个 skill（加上已有的 `design-taste-frontend`，`<available_skills>` 共 10 个条目）。按层级标注 Tier 2 / Tier 3。
...
- ⚠️ 页面运行时转换（prototype ↔ high-fidelity-react）时此规则不适用
- 完整规则（Props 声明、依赖策略）：先用 `readPreinstalledSkill({ name: 'react-high-fidelity' })` 读取；转换场景用 `readPreinstalledSkill({ name: 'page-runtime-conversion' })`
```
...
#### Skill 9: `page-runtime-conversion` — 页面运行时类型转换（Tier 2）
...
- 用户触发页面运行时类型切换（prototype-html-css ↔ high-fidelity-react）时，以源页面当前渲染效果为视觉 ground truth
- 不得用 @preview/sdk 通用组件替换源页面的自定义视觉
- 完整转换规范：先用 `readPreinstalledSkill({ name: 'page-runtime-conversion' })` 读取
```
...
**与 `buildRuntimeConversionPrompt` 的关系**：方案实施后，`page.tsx` 中的 `buildRuntimeConversionPrompt` 简化为仅携带页面元信息（pageId、pageName、sourceRuntimeType、targetRuntimeType）+ 触发 skill 指令，完整规则全部从 skill 按需加载。不再维护两套独立的转换提示词。
...
│   └── SKILL.md
└── page-runtime-conversion/  # Skill 9 (Tier 2)
    └── SKILL.md
```
...
4. **不移除** Tier 1 内容（含子 Agent 委派，约 28 行）
5. 重新编号和组织剩余内容
...
- **新增**：`page.tsx` 中 `buildRuntimeConversionPrompt` 从约 45 行内联规则简化为仅携带页面元信息（pageId、pageName、sourceRuntimeType、targetRuntimeType）+ 触发 `readPreinstalledSkill("page-runtime-conversion")` 的指令
...
- **新增 skill 加载测试**：在 `agent-service` 测试中验证 `getPreinstalledSkills()` 返回所有 9 个内置 skill，frontmatter 解析正确
...
- 手动验证：启动 agent-service，确认 `<available_skills>` 正确列出所有内置 skill（10 个，含 design-taste-frontend）
...
- [x] 确认 `external-collaboration` 条件注册的触发时机（运行时工具列表 vs 环境变量）
- [x] 确认 `react-high-fidelity` 内容边界（Schema 规则保留 Tier 1，仅 React 特有规则入 skill）
- [x] 确认 `page-runtime-conversion` 作为 Skill 9 纳入实施方案
- [ ] 确认 `buildRuntimeConversionPrompt` 简化方案（页面元信息 + 触发词）
- [ ] 实施阶段一：新建 SKILL.md 文件（9 个）
- [ ] 实施阶段二：扩展 preinstalled-skills.ts（目录扫描 + source 字段 + 条件注册 + 缓存清理）
- [ ] 实施阶段三：裁剪 system-prompt.md（Tier 2 保留最小约束，Tier 3 仅留引用指令；config.schema.json 规则保留 Tier 1）
- [ ] 实施阶段四：简化 system-prompt.ts（移除条件渲染，改为 skill 条件注册）+ 简化 page.tsx 中 buildRuntimeConversionPrompt
...
| `<available_skills>` 膨胀 | 抵消瘦身收益 | 10 个条目（含已有 design-taste-frontend）约 50 行 XML，可接受；条件注册的 skill 在工具不可用时不出现 |
...
| `page-runtime-conversion` skill 不被触发 | 转换行为回退到默认 React 规范 | `buildRuntimeConversionPrompt` 中显式要求调用 `readPreinstalledSkill("page-runtime-conversion")`；基座 Tier 2 最小约束中保留转换场景的核心禁止规则（不得用 @preview/sdk 替换自定义视觉） |
| `react-high-fidelity` 与 `page-runtime-conversion` 边界模糊 | agent 在创建 React 页时错误地读取转换 skill，或反之 | 两个 skill 的 `description` 和触发词明确互斥；`react-high-fidelity` 基座约束注明「本规范仅适用于新建/重写」并指向 `page-runtime-conversion`；`page-runtime-conversion` 基座约束注明「用户触发运行时切换时」 |
