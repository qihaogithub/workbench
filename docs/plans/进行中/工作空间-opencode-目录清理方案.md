...
function injectOpencodeAgentConfig(workspacePath: string, projectId: string): void {
  ...
  const template = fs.readFileSync("demo-generator.template.md", "utf-8");
  const agentMd = template.replace(/\{\{PROJECT_NAME\}\}/g, projectName)...;
...
- ⚠️ `.opencode/` 目录中的 `references/` 子目录仍被提示词引用（`references/config-system.md`）
...
**决策**：清理 `.opencode/` 目录，但保留 `references/` 注入逻辑
...
3. `references/` → 配置系统参考文档（`config-system.md` 等），仍被 `DEMO_GENERATOR_TEMPLATE` 引用（L138："必须先读取 references/config-system.md"）
...
| 文件 | 改动类型 | 说明 |
|:-----|:---------|:-----|
| `packages/author-site/src/lib/workspace-manager.ts` | 重构 | 移除 `injectOpencodeAgentConfig()` 中的 `.opencode/` 注入逻辑，保留 `injectReferences()` |
...
**目标**：工作空间创建时不再注入 `.opencode/` 目录

| 步骤 | 文件 | 改动 |
|:-----|:-----|:-----|
| 1 | `workspace-manager.ts` L49-128 | 将 `injectOpencodeAgentConfig()` 重构为 `injectWorkspaceReferences()` |
| 2 | 移除 opencode.json 写入逻辑 | 删除 L63-85（写入 opencode.json 的代码） |
| 3 | 移除 demo-generator.md 注入逻辑 | 删除 L87-127（读取模板、替换占位符、写入 agent md） |
| 4 | 保留 injectReferences() 调用 | L60-61 保留（注入 references/ 目录） |
| 5 | 运行 `pnpm typecheck` | 验证编译通过 |

**重构后代码**：
```typescript
/**
 * 注入工作空间参考文件（references/）
 * 提供配置系统文档等参考资料，供 AI 在生成 config.schema.json 时查阅
 */
function injectWorkspaceReferences(workspacePath: string): void {
  injectReferences(workspacePath);
}
```
...
| 收益 | 说明 |
|:-----|:-----|
| **代码量减少** | 删除 ~80 行 `.opencode` 注入逻辑（workspace-manager.ts L63-127） |
| **存储节省** | 每个新工作空间减少 ~2KB（opencode.json + demo-generator.md） |
| **架构一致性** | 工作空间目录不再包含废弃的 OpenCode 配置 |
| **维护简化** | 移除模板占位符替换逻辑（projectName/pageCount/pageList 等） |
...
| 风险 | 概率 | 影响 | 缓解措施 |
|:-----|:-----|:-----|:---------|
| **references/ 注入遗漏** | 低 | 高 | 阶段一保留 `injectReferences()` 调用，阶段四重点验证 |
...
author-site/src/lib/workspace-manager.ts
  ...
  │   ├─ fs.cpSync(projectWorkspacePath, workspacePath)  ← 复制项目文件
  │   ├─ 写入 .workspace.json  ← 元数据
  │   └─ injectWorkspaceReferences()  ← 重构后（原 injectOpencodeAgentConfig）
  │       └─ injectReferences()  ← 注入 references/ 目录
...
| `references/` | 工作空间参考文件目录，包含 config-system.md 等配置系统文档 |
| `injectOpencodeAgentConfig()` | 原函数名，负责注入 .opencode 配置和提示词 |
| `injectWorkspaceReferences()` | 重构后函数名，仅负责注入 references/ 参考文件 |
