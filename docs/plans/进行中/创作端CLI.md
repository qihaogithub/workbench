...
CLI 与创作端能力对齐仍需长期跟踪。最近更新：2026-07-04。

主线结论：CLI 对账未发现新的共享层能力缺口，也未发现全命令测试漏覆盖问题。`commands --json` 与 `register(...)` 列表保持一致，`cli-all-commands` 末尾仍用反查守卫覆盖所有已注册命令。2026-07-04 新确认的变化是内容图与资源历史能力已经进入当前工作树：`project commit-list`、`project materialize`、`project content-gc` 与 `resource version-*` 已完成共享层承载和 CLI 覆盖；本轮差异仍然是低风险文档漂移，不是命令缺口。剩余 4 个结构性缺口继续维持只报告策略。
