...
> 版本：v2.0
> 创建日期：2026-05-03
> 更新日期：2026-05-03（v2.0：重构编辑页布局与AI交互模型）
> 状态：方案设计中，现阶段仅规划不实施
...
```
"一个项目 = 多个页面，AI 全域编辑，自然语言驱动，版本以项目为单位"
```
...
用户新建项目
      │
      ▼
创建项目目录和 project.json（demoPages 初始为空）
      │
      ▼
...
用户打开项目编辑
      │
      ▼
创建 Session + Workspace（复制整个 workspace/ 含 demos/ 子目录）
      │
      ▼
AI 代理获得整个工作空间（所有 demos/）的读写权限
      │
      ▼
用户通过自然语言告诉 AI 操作哪个页面：
  "帮我修改首页的标题为'欢迎'"
  "给详情页增加一个配置项"
  "新建一个'关于我们'页面"
  "删除'测试页'那个页面"
      │
      ▼
AI 自主定位 target demo 目录并操作文件
如果是页面管理指令（新建/删除/重命名），AI 同步更新 demoPages 元数据
      │
      ▼
文件变更追踪器自动检测所有页面的文件变化
右侧面板实时显示所有变更文件列表（按页面分组）
      │
      ▼
用户保存 → 所有 Demo 作为集体快照保存为统一版本
```
...
清理系统文件（.opencode、.session.json、.workspace.json）
      │
      ▼
更新 project.json 中的 demoPages 列表（反映新增/删除的 Demo）
      │
      ▼
记录版本信息，清理旧版本
      │
...
│     和 config.schema.json。你可以在「页面」Tab 中查看。               │
│                                                                       │
│  ─────────────────────────────────────────────────────────────────   │
│                                                                       │
│  ┌──────────────────────────────────────────────────┐                │
...
> **重要**：AI 执行页面创建/删除/重命名操作时，需要同步更新 `project.json` 中的 `demoPages` 数组。这些写入操作属于编辑会话内操作，会在用户保存时一并合并到正式项目。
...
│  │     ✨  demos/detail/config.schema.json  （新增）           │   │
│  │                                                              │   │
│  │  📄 产品中心 (新增)                                          │   │
│  │     ✨  demos/demo_001/index.tsx                             │   │
│  │     ✨  demos/demo_001/config.schema.json                    │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─ 备注 ──────────────────────────────────────────────────────┐   │
│  │  ┌──────────────────────────────────────────────────────┐   │   │
