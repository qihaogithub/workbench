...
> 版本：v4.0
> 创建日期：2026-05-03
> 更新日期：2026-05-03（v4.0：新增架构评估与预览双模式设计）
> 状态：方案设计中，现阶段仅规划不实施
...
5. **禁止遗漏任何页面**
6. 更新 project.json 的 hasProjectConfig = true
...
| **配置持久化** | 配置值保存在独立的数据文件中（如 workspace/config.data.json），与 Schema 文件分离 |
| **表单生成** | 根据 Schema 定义自动生成对应的表单控件（文本 → 输入框、图片 → 上传组件、颜色 → 取色器） |
| **配置值校验** | 根据 JSON Schema 的 type/enum/format 等约束校验用户输入 |

**配置值的存储：**

Schema 定义（project.config.schema.json / page config.schema.json）与 配置值数据分离存储：

```
workspace/
├── project.config.schema.json   ← 定义"有什么配置项"
├── project.config.data.json     ← 存储"配置项填了什么值"（用户填写）
└── demos/home/
    ├── config.schema.json       ← 定义"有什么配置项"
    ├── config.data.json         ← 存储"配置项填了什么值"（用户填写）
    └── index.tsx
```

优势：Schema 由 AI 维护（编辑时），Data 由用户填写（使用时），互不干扰。
...
3. 如果所有共享字段都被删除，删除 project.config.schema.json，设 hasProjectConfig = false
...
- ❌ 修改 project.json 中除 demoPages 和 hasProjectConfig 外的其他字段
...
│   [project.json — hasProjectConfig = true]                   │
│                                                             │
...
| `GET` | `/api/projects/[projectId]/config/data` | 获取项目级配置值（用户填写的值） |
...
| 方法 | 路由 | 变更 |
|:-----|:-----|:-----|
| `GET` | `/api/sessions/[sessionId]/files` | 返回结构需包含所有 Demo 的文件映射 + 项目配置文件 |
| `GET` | `/api/projects/[projectId]` | 返回结果包含 `demoPages` 和 `hasProjectConfig` |
...
| 任务 | 说明 |
|:-----|:-----|
| 定义新类型 | DemoPageMeta、DemoPageDetail、ProjectConfig、MergedComponentProps、CreateDemoPageRequest、UpdateDemoPageRequest |
| 更新 Project | 添加 `demoPages: DemoPageMeta[]` 和 `hasProjectConfig: boolean` 字段 |
...
| 新增项目配置 fs-utils | `getProjectConfig(projectId)` — 读取 project.config.schema.json；`saveProjectConfig(projectId, schema)` — 写入 project.config.schema.json；`deleteProjectConfig(projectId)` — 删除项目配置；`getProjectConfigData(projectId)` — 读取 config.data.json；`saveProjectConfigData(projectId, data)` — 保存配置值 |
| 更新 ensureWorkspaceFiles | 支持创建 `demos/default/` 目录结构 |
| 更新 createProject | 默认创建 `demos/default/` 及默认文件；初始 hasProjectConfig = false |
...
| 更新 projectApiClient | 添加对应的前端封装方法（Demo + Config） |
...
| 任务 | 说明 |
|:-----|:-----|
| 重写 agent.md 提示词 | 定义工作空间目录结构（含项目配置）；说明页面信息获取方式；支持自然语言页面定位；新增页面管理操作指令；**新增项目级配置管理指令**（创建/删除/修改共享配置字段）；移除单目录范围限制 |
| 更新 opencode.json | 调整 tools 权限以允许跨 Demo 文件操作和 project.json/project.config.schema.json 写入 |
...
| **配置面板适配** | 配置面板新增项目配置区（与页面配置区分层展示）；切换预览页面时项目配置区不变；配置值与 Schema 分离存储 |
| **预览双模式 UI** | 预览区顶部增加「单页模式/宫格模式」切换按钮；宫格模式工具栏（每行数量下拉框、返回单页按钮） |
| **宫格渲染实现** | CSS Grid 布局；iframe + transform:scale 缩放渲染；滚动定位逻辑（scrollIntoView） |
...
| **AI 未同步所有页面组件** | 新增共享配置字段后，部分页面未更新 Props | agent.md 强约束：新增/删除项目配置字段时**必须**更新所有页面；代码审查检查点 |
| **配置字段命名冲突** | 项目配置和页面配置有同名字段，用户预期不一致 | 合并机制明确：页面级优先覆盖；配置面板视觉区分来源；AI 设计时避免冲突 |
| **宫格模式性能问题** | 多个 iframe 同时渲染，占用内存和 CPU | 宫格模式仅在需要时使用；页面数量超过 10 个时限制 iframe 数量；考虑懒加载 |
...
| **编辑页三 Tab 状态管理复杂** | 切换 Tab 时状态不同步 | 页面列表 + 项目配置状态作为全局共享状态；Tab 间通过 Context 保持同步 |
