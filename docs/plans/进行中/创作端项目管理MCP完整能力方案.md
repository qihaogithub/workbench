...
- [ ] 新增 `packages/project-core/`。
- [ ] 从 author-site 提取项目、模板、页面、文件夹、配置核心服务。
- [ ] 定义统一错误模型、操作结果、差异摘要和验证结果。
- [ ] Web API 改为调用核心服务的薄封装。
- [ ] 补齐 project-core 单元测试。
- [ ] 建立兼容测试，确认 Web API 响应结构不变。

### 阶段 2：MCP Server 骨架

- [ ] 新增 `packages/project-admin-mcp/`。
- [ ] 接入 project-core。
- [ ] 实现鉴权、操作者身份、工具注册和统一响应。
- [ ] 实现 `project_*`、`template_*`、`edit_*` 基础工具。
- [ ] 记录审计日志。
- [ ] 提供 stdio 启动方式和 Codex 本地配置示例。
- [ ] 提供安装提示词生成函数，供 Web 介绍页复用。
...
- [ ] 实现 `preview_*` 工具。
- [ ] 实现 `publish_*` 工具。
- [ ] 串联 screenshot-service 健康检查。
- [ ] 建立发布前检查清单。

### 阶段 5：AI、审计、技能与团队使用

- [ ] 实现 `ai_*` 和 `audit_*` 工具。
- [ ] 新增 `opencode-project-admin` 技能。
- [ ] 编写团队使用说明。
...
- [ ] Web 端能创建的资源，MCP 都能创建。
- [ ] Web 端能修改的项目结构，MCP 都能修改。
- [ ] Web 端能触发的验证，MCP 都能触发。
- [ ] MCP 写入后的项目可以被 Web 端无报错读取。
- [ ] MCP 写入后的项目可以被使用端无报错读取。
- [ ] MCP 不能写入 Web 端无法识别的内部状态。
- [ ] 所有 L3/L4 操作都有二段确认和审计。
- [ ] 所有 UI 相关变更都有编译结果，必要时有截图。
- [ ] 所有 Schema 相关变更都有合并校验。
- [ ] 所有发布相关变更都有发布前检查。
- [ ] 首页存在 MCP 入口，并能跳转介绍页。
- [ ] 介绍页能复制 Codex 安装提示词。
- [ ] 安装提示词不包含真实密钥。
- [ ] 安装提示词能引导 Codex 完成只读自检。
