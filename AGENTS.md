...
`packages/agent-service/AGENTS.md` 包含详细的 ACP 协议说明和支持的后端列表 — 修改 agent-service 前应阅读。
...
**默认后端是 `opencode-http`** (HTTP 直连 `OPENCODE_SERVER_URL`)。旧 ACP `opencode` 后端已废弃，仅在 `server.ts:62` 保留兼容注册。完整后端列表见 `packages/agent-service/src/server.ts:62-` 的 `factory.register` 调用。ACP 后端 (claude, codex, gemini 等) 通过 stdio 子进程通信。
