...
interface DemoProps {
  ...
  svgaSrc?: string;
  spineSrc?: string | { skeleton?: string; atlas?: string; texture?: string };
  spineLoop?: boolean;
  riveSrc?: string;
  riveFit?: string;
...
export default function MotionFormats(props: DemoProps) {
  ...
  // Spine 素材以 .zip 上传，服务端解压后返回各文件 URL 对象
  const spineFiles =
    spineSrc && typeof spineSrc === "object"
      ? {
          skeleton: toLocalPath((spineSrc as { skeleton?: string }).skeleton),
          atlas: toLocalPath((spineSrc as { atlas?: string }).atlas),
          texture: toLocalPath((spineSrc as { texture?: string }).texture),
        }
      : null;
  const hasSpine = !!(spineFiles?.skeleton && spineFiles?.atlas && spineFiles?.texture);
  ...
  return (
    <div className="min-h-screen px-3 py-5 space-y-4" style={{ backgroundColor: BG_COLOR }}>
      ...
      {/* Spine 动效 */}
      <Card icon="🦴" title="Spine 动效" tag="Spine .zip">
        {hasSpine ? (
          <div className="flex items-center justify-center h-40 rounded-xl bg-slate-900/60 overflow-hidden">
            <SpinePlayer
              ...
              fallback={
                <div className="text-xs text-slate-400 px-4 text-center">
                  Spine 素材加载中或不可用，请检查 .zip 素材是否包含骨架、图集与纹理
                </div>
              }
            />
          </div>
        ) : (
          <Placeholder text="在右侧面板「Spine 动效」上传 .zip 打包素材（骨架+图集+纹理）" />
        )}
      </Card>
