...
export default function Page(props: DemoProps) {
  ...
  const bannerPos = pos.bannerImage ?? { x: 20, y: 20 };
  const actionPos = pos.bottomBtn ?? { x: 20, y: 220 };
      ...
      <div className="mx-auto flex w-full max-w-md flex-col" style={{ gap: `${gapPx}px` }}>
        ...
        <section className="space-y-3">
          ...
          {safeBlocks.map((block, i) => {
            if (block.type === 'bannerPos' || block.type === 'actionPos') {
              const isBanner = block.type === 'bannerPos';
              const p = isBanner ? bannerPos : actionPos;
              return (
                <div key={i}>
                  <div className="mb-2 flex items-center justify-between px-1">
                    <span className="text-xs font-semibold" style={{ color: muted }}>
                      {isBanner ? '横幅图片定位模块' : '底部按钮定位模块'}
                    </span>
                    {renderTag(isBanner ? 'bannerImage' : 'bottomBtn')}
                  </div>
                  <div
                  ...
                  >
                    {isBanner ? (
                      block.pic ? (
                        ...
                        <span
                          className="text-xs"
                          style={{ position: 'absolute', left: p.x, top: p.y, opacity: 0.45 }}
                        >
                          （未设置图片）
                        </span>
                      )
