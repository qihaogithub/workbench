...
const SVG_MOTIONS: Record<string, string> = {
  float: "float",
  pulse: "pulse",
  rotate: "rotate",
  none: "none",
...
export default function MotionFormats(props: DemoProps) {
  const {
    pageTitle = "动效格式展示",
    subtitle = "SVG · Lottie · SVGA · Spine · Rive",
    bgColor = "#0f172a",
    accentColor = "#6366f1",
  } = props;

  const svg = props["SVG 动效"] || {};
  const lottie = props["Lottie 动效"] || {};
  const svga = props["SVGA 动效"] || {};
  const spine = props["Spine 动效"] || {};
  const rive = props["Rive 动效"] || {};

  const svgSrc = (svg.src as string) || "";
  const svgMotion = (svg.motion as string) || "float";
  const svgDuration = (svg.duration as number) || 3;
  ...
  const svgAnim =
    svgMotion !== "none"
      ? { animation: `${SVG_MOTIONS[svgMotion]} ${svgDuration}s ease-in-out infinite` }
      : undefined;
  ...
  return (
    <div className="min-h-screen px-3 py-5 space-y-4" style={{ backgroundColor: bgColor }}>
      <style>{`
      ...
      {/* SVG 动效 */}
      <Card icon="🖼️" title="SVG 动效" tag="SVG .svg">
        {svgSrc ? (
          ...
          <Placeholder text="在右侧面板「SVG 动效」中上传 SVG 资源，可在浮动 / 脉冲 / 旋转间切换" />
        )}
      </Card>
      ...
      >
        ...
        <p>
          五种动效格式均通过系统播放器渲染：SVG 使用 CSS 关键帧动画，Lottie / SVGA / Spine /
          Rive 使用对应播放组件。素材在右侧配置面板中上传后即可实时替换预览。
        </p>
      </div>
    </div>
  );
