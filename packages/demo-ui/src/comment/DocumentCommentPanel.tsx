...
export function DocumentCommentPanel({
  ...
  activeThreadId, onSelectThread, onCreateComment, selectionDraft, onSelectionDraftHandled, className,
  ...
  return <div className={cn("relative flex h-full min-h-0 flex-col", className)}>
    <CommentSidebar threads={threads} currentUserId={currentUserId} activeThreadId={activeThreadId} onSelectThread={onSelectThread} showHeader={false} className="min-h-0 flex-1" />
    ...
    {creating && <CommentCreatePopover draft={draft} mentionCandidates={candidates} canMentionAgent={canMentionAgent} left={160} top={80} onCancel={() => setCreating(false)} onSubmit={async (input) => { await onCreateComment(input); setCreating(false); }} />}
  </div>;
}
