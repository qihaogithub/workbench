...
vi.mock(
  "../../src/backends/pi-tools/global-image-store",
  () => imageStoreTestMocks,
);
...
describe("PiAgentBackend", () => {
  ...
  describe("模型信息", () => {
    it("应返回当前模型信息", async () => {
      ...
      expect(modelInfo?.currentModelId).toBe(
  ...
  describe("消息发送", () => {
        ...
        },
        {
          messages: [
            { id: "old", role: "user", content: "应被裁掉" },
            {
              id: "kept-1",
              role: "user",
              content: [{ type: "text", text: "保留的用户消息" }],
            },
            {
              role: "assistant",
              content: [{ type: "text", text: "保留的助手消息" }],
            },
    ...
    it("在发送前达到安全阈值时压缩历史上下文", async () => {
      ...
      await expect(backend.sendMessage("x".repeat(420_000))).resolves.toBe(
        "ok",
      );
    ...
    it("上下文溢出后仅压缩并重试原请求一次", async () => {
      ...
      await expect(backend.sendMessage("retry this")).resolves.toBe(
        "recovered",
      );

      expect(harness.compact).toHaveBeenCalledTimes(1);
      expect(harness.prompt).toHaveBeenCalledTimes(2);
      expect(harness.prompt.mock.calls[1]).toEqual(
        harness.prompt.mock.calls[0],
      );
    });
    ...
    it("应提取字符串形式的 AssistantMessage content", async () => {
      ...
      await expect(backend.sendMessage("hello")).resolves.toBe(
        "plain text response",
      );
    });
    ...
    it("后续消息应注入当前会话已上传文件清单", async () => {
      ...
      fsMocks.promises.readFile.mockResolvedValue(
        JSON.stringify({
          id: "att-history",
          name: "商城首页.html",
          mimeType: "text/html",
    ...
    it("底层发送失败日志应保留 Error 的可诊断字段", async () => {
      ...
      piAgentMocks.harnesses[0].prompt = vi
        .fn()
    ...
    it("应在 updatePlan 成功后发出结构化 plan 事件，失败时不覆盖", async () => {
      ...
      harness.subscriber?.({
        ...
        result: {
          ...
          details: {
            success: false,
            items: [{ id: "failed", title: "失败计划", status: "failed" }],
          },
        },
      });
  ...
  describe("子 agent", () => {
    it("应执行短生命周期子 agent 并汇总写文件变更", async () => {
      ...
      expect(
        piAgentMocks.harnesses[0].options.tools.map((tool: any) => tool.name),
      ).not.toContain("delegateTask");
      expect(piAgentMocks.harnesses[0].options.resources.skills[0].name).toBe(
        "page-lifecycle",
      );
    ...
    it("主 agent 将项目规则放入不可覆盖的安全骨架后，并注入运行时工具", async () => {
      ...
      const prompt = await harness.options.systemPrompt({
        ...
        resources: harness.options.resources,
      });
    ...
    it("禁用子 agent 时应拒绝委派", async () => {
      ...
      await expect(
        (backend as any).runSubagent({ task: "noop" }),
      ).rejects.toThrow("Subagents are disabled");
    });
    ...
    it("vision 模式下载 /api/images/ 应从本地图床读取", async () => {
      ...
      expect(result.success).toBe(true);
      expect(imageStoreTestMocks.readGlobalImageById).toHaveBeenCalledWith(
        "img_test123",
      );
    });
...
describe("PiAgent 工具", () => {
  ...
  describe("createWorkbenchTools", () => {
    it("应创建所有必要的工具", async () => {
      const { createWorkbenchTools } = await import(
        "../../src/backends/pi-tools"
      );
      ...
      expect(toolNames).toContain("observePreview");
      ...
      expect(toolNames).toContain("requestUserChoice");
    ...
    it("readPreinstalledSkill 应读取内置 page-lifecycle skill", async () => {
      ...
      const readSkill = tools.find(
        (tool) => tool.name === "readPreinstalledSkill",
      )!;
      ...
      expect(result.content[0].text).toContain(
        "Source: workbench:internal:page-lifecycle",
      );
    ...
    it("readPreinstalledSkill 找不到 skill 时返回结构化错误", async () => {
      ...
      expect(result.content[0].text).toContain(
        "Available skills: page-lifecycle",
      );
    });
    ...
    it("工具能力应来自真实工具注册列表", async () => {
      const { getWorkbenchToolCapabilities, WORKBENCH_TOOL_VERSION } =
        await import("../../src/backends/pi-tools");
    ...
    it("使用端只读工具能力不应包含写入或执行工具", async () => {
      const { getViewerReadonlyToolCapabilities } = await import(
        "../../src/backends/pi-tools"
      );
