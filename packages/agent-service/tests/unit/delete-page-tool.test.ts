...
describe('page management tools', () => {
  ...
  it('deletes the last page and leaves an empty workspace tree', async () => {
    ...
    expect(result.isError).toBeFalsy();
    expect(permission).toHaveBeenCalledTimes(1);
    expect(fs.existsSync(path.join(tmpDir, 'demos', 'page_only'))).toBe(false);
    expect(readTree().pages).toEqual([]);
  });
  ...
  it('previews and executes explicit IDs that delete the last page', async () => {
    writeTree([
    ...
    expect(preview.details.canExecute).toBe(true);
    expect(preview.details.reason).toBeUndefined();
    expect(preview.details.remainingCount).toBe(0);
