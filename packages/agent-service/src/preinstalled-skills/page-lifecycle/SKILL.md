...
- `field` 只能引用当前对象作用域内的兄弟字段；对象数组的每个条目独立求值，`items.oneOf` / `variants` 分支和详情 Sheet 使用同一规则
- 新 Schema 统一把 `visibleWhen` 直接声明在字段上；解析器同样支持 `ui:options.visibleWhen`。在两种位置之间移动同一声明不会改变运行时行为，不能把移动声明位置当作显隐修复；不要同时写两处
- 被隐藏字段的当前值不会丢失，重新显示后恢复
- `schemaValidate` 只验证 Schema 语法和 Workbench 契约，不验证当前运行中的配置面板。若任务要求实际显隐效果，必须在真实界面切换控制字段并验证隐藏、重显和数值恢复；未完成界面验证时，只能报告“修改已写入、效果待验证”，不能宣称“已修复”
