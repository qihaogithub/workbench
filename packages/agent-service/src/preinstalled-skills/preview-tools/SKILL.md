---
name: preview-tools
description: 预览调试（控制台日志、可选截图）与画布管理（arrangeCanvasPages）的完整规则。触发词：调试预览、获取控制台日志、截图、页面报错、白屏排查、整理画布、排列画布页面、调整画布位置。
---
...
如果运行时工具目录中提供了 `captureScreenshot`，可以获取当前页面的 PNG 截图，用于检查视觉效果、布局和响应式问题。截图依赖独立服务和 Chromium 健康检查，不可用时不要创建截图任务或反复重试；功能实现、控制台排查和文字验收仍可继续：
