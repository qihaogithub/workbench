...
import { AcpConnection, AcpSessionUpdate, AcpPermissionRequest, AcpModelInfo } from '../acp';
...
export class OpenCodeAcpBackend implements IBackendAdapter {
  ...
  private fullContent = '';
  private files: Array<{ path: string; action: 'created' | 'modified' | 'deleted'; content?: string }> = [];
  ...
  async initialize(): Promise<void> {
    ...
    this.status = 'ready';
    logger.info({ sessionId: this.connection.currentSessionId }, 'OpenCode ACP backend initialized');
  }
  ...
  async start(options?: { resumeSessionId?: string }): Promise<void> {
    ...
    if (options?.resumeSessionId) {
      try {
        ...
        logger.warn({ error, resumeSessionId: options.resumeSessionId }, 'Failed to resume session, creating new one');
        await this.connection.createSession({ model: this.config.model });
      }
    } else {
      await this.connection.createSession({ model: this.config.model });
    ...
    this.status = 'ready';
    logger.info({ sessionId: this.connection.currentSessionId }, 'OpenCode ACP backend started');
  }
  ...
  private async handlePermissionRequest(request: AcpPermissionRequest): Promise<{ optionId: string }> {
    if (this.connection) {
    ...
    if (selected) {
      logger.info({ toolCallId: request.toolCall.toolCallId, option: selected.name }, 'Auto-approving permission');
  ...
  async destroy(): Promise<void> {
    ...
    this.status = 'idle';
    logger.info({ sessionId: this.config.sessionId }, 'OpenCode ACP backend destroyed');
  }
