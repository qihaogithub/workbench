...
/** 右侧栏「素材」页签：配置项素材池（按受影响页面分组，可折叠/筛选/拖拽绑定） */
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
        {ws.poolGroups.length === 0 ? (
          <div className="py-8 text-center text-xs text-muted-foreground">
            无匹配配置项
          </div>
        ) : (
          ws.poolGroups.map(([groupName, items]) => {
            const collapsed = ws.collapsedGroups.has(groupName);
            return (
              <div key={groupName} className="mb-1">
                <div
                  className="flex cursor-pointer select-none items-center gap-1.5 px-1.5 py-1 text-sm font-semibold"
                  onClick={() => ws.toggleGroup(groupName)}
                >
                  <ChevronDown
                    className={cn(
