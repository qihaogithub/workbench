...
describe("HoverPop", () => {
  it("为非图片配置项仅显示所在页面示意图", () => {
    render(<HoverPop pop={{ item: baseItem, x: 10, y: 20 }} />);

    expect(screen.getByAltText("所在页面示意")).toBeInTheDocument();
    expect(screen.queryByAltText("主标题")).not.toBeInTheDocument();
    expect(screen.queryByText("主标题")).not.toBeInTheDocument();
    expect(screen.queryByText("首页")).not.toBeInTheDocument();
  });

  it("为图片配置项同时显示默认图和所在页面示意图", () => {
    ...
    render(<HoverPop pop={{ item: imageItem, x: 10, y: 20 }} />);
