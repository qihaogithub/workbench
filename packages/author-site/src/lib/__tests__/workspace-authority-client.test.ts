...
describe("workspace authority client", () => {
  ...
  it("EXTERNAL_DRIFT 原样失败，不隐式 reconcile adopt", async () => {
    const driftResponse = {
      ok: false,
      status: 409,
      json: async () => ({ success: false, error: { code: "WORKSPACE_EXTERNAL_DRIFT", message: "drift" } }),
    };
    global.fetch = jest.fn().mockResolvedValueOnce(driftResponse) as unknown as typeof fetch;
    ...
    await expect(commitWorkspaceMutation(request)).rejects.toMatchObject({ code: "WORKSPACE_EXTERNAL_DRIFT" });
    expect(global.fetch).toHaveBeenCalledTimes(1);
  });

  it("Authority 冲突原样失败，不执行隐式恢复", async () => {
    const driftResponse = {
      ok: false,
      status: 409,
      json: async () => ({ success: false, error: { code: "WORKSPACE_EXTERNAL_DRIFT", message: "drift" } }),
  ...
  it("EXTERNAL_DRIFT 不触发 reconcile 请求", async () => {
    const driftResponse = {
      ok: false,
      status: 409,
      json: async () => ({ success: false, error: { code: "WORKSPACE_EXTERNAL_DRIFT", message: "drift" } }),
