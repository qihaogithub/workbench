...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ projectId: string; documentId: string }> }) {
  ...
  try {
    ...
    const data = await createDocumentApplicationService().listRevisions({ projectId, documentId }, actor);
