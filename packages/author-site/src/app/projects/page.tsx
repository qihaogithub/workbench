...
import {
  ...
  UsernameDisplay,
} from "@/components/username-selector";
...
export default function ProjectsPage() {
  ...
  const handleEditProject = async (projectId: string) => {
    ...
    try {
      const result = await projectApiClient.openProjectEdit(projectId, {
        username,
      });
      // 跳转到编辑页面，传递会话信息
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      {/* 用户名选择器 */}
      {!username && (
        <UsernameSelector onUsernameChange={handleUsernameChange} />
      )}
      ...
      <div className="mb-8 p-6 bg-card rounded-lg border">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          <div className="p-4 border rounded-md">
            ...
            <p className="text-sm text-muted-foreground">
              文本输入，支持 maxLength、minLength、pattern 等验证
            </p>
          </div>
          <div className="p-4 border rounded-md">
            <h3 className="font-medium mb-2">🔢 数字 (number/integer)</h3>
            <p className="text-sm text-muted-foreground">
              数字输入，支持 minimum、maximum 范围限制
            </p>
          </div>
          <div className="p-4 border rounded-md">
        ...
        <div className="mt-4 p-3 bg-muted rounded-md">
          <p className="text-sm text-muted-foreground">
            💡 提示：访问{" "}
            <a href="/demo-test" className="text-primary hover:underline">
              Demo 测试页面
            </a>{" "}
            查看配置面板的实际效果
          </p>
