...
export interface HtmlImportPrototypeGateResult {
  ok: boolean;
  reasonCodes: string[];
}
export function validateHtmlImportPrototypeCandidate(
  html: string,
  css: string,
  ...
  if (css.length > MAX_PROTOTYPE_CSS_LENGTH)
    reasonCodes.push("PROTOTYPE_CSS_TOO_LARGE");
  if (/<\s*script\b/i.test(html))
    reasonCodes.push("PROTOTYPE_SCRIPT_FORBIDDEN");
  if (/\son[a-z]+\s*=/i.test(html))
    reasonCodes.push("PROTOTYPE_INLINE_EVENT_FORBIDDEN");
  if (/javascript\s*:/i.test(html) || /javascript\s*:/i.test(css))
    reasonCodes.push("PROTOTYPE_JAVASCRIPT_URL_FORBIDDEN");
  if (/<\s*(iframe|embed|object)\b/i.test(html))
    reasonCodes.push("PROTOTYPE_EMBED_FORBIDDEN");
  if (
    /<\s*form\b[^>]*\baction\s*=/i.test(html) ||
    /<\s*form\b[^>]*\bmethod\s*=\s*["']?post\b/i.test(html) ||
    /<(?:button|input)\b[^>]*\btype\s*=\s*["']?(?:submit|image)\b/i.test(html)
  )
    reasonCodes.push("PROTOTYPE_FORM_ACTION_FORBIDDEN");
  if (/@import\b/i.test(css))
    reasonCodes.push("PROTOTYPE_CSS_IMPORT_FORBIDDEN");
  if (/(^|[,{;]\s*)(html|body|:root)\b/i.test(css))
    reasonCodes.push("PROTOTYPE_GLOBAL_SELECTOR_FORBIDDEN");
...
const DATA_SCRIPT_TYPES =
  /^(?:application|text)\/(?:ld\+json|json|json5|applicationmanifest\+json|sql|graphql)$/i;
...
/** A Figma export needs both its marker and valid fixed-canvas dimensions. */
function detectFigmaExportViewport(
  source: string,
  ...
  const inline = source.match(
    /<[^>]+class=["'][^"']*\bfigma-export\b[^"']*["'][^>]*style=["'][^"']*?width\s*:\s*([\d.]+)px\s*;?[^"']*?height\s*:\s*([\d.]+)px/i,
  );
...
function addCssReferences(
  css: string,
  pathName: string,
  resources: HtmlResourceReference[],
  capabilities: HtmlUnsupportedCapability[],
  tagName = "style",
): number {
  let total = 0;
  for (const match of css.matchAll(
    /@import\s+(?:url\(\s*)?["']?([^"'\s)]+)|url\(\s*["']?([^"'\s)]+)/gi,
  )) {
    ...
    resources.push({
      tagName,
      attributeName: "css-url",
      value,
      classification,
      ...resourcePolicy(classification),
      path: pathName,
    });
    if (classification !== "fragment" && classification !== "data")
      capabilities.push({
    ...
    if (
      classification === "data" &&
      dataBytes(value) > HTML_IMPORT_MAX_DATA_URL_BYTES
    )
      capabilities.push({
        code: "data-url-too-large",
        path: pathName,
        detail: `${dataBytes(value)}`,
      });
  }
  return total;
}
...
function canonicalize(node: HtmlNode, removeInertScripts: boolean): void {
  ...
  if (node.childNodes) {
    node.childNodes = node.childNodes.filter((child) => {
      if (!removeInertScripts || child.tagName?.toLowerCase() !== "script")
        return true;
      const type =
        child.attrs
          ?.find((a) => a.name.toLowerCase() === "type")
          ?.value.trim()
          .toLowerCase() ?? "";
...
export function normalizeHtmlImport(source: string): HtmlImportNormalization {
  ...
  const empty = (
    outcome: HtmlImportAnalysis["outcome"],
  ...
  if (!source.trim())
    return empty({ status: "rejected", code: "HTML_IMPORT_INVALID" });
  if (Buffer.byteLength(source, "utf8") > HTML_IMPORT_MAX_INPUT_BYTES)
    return empty({ status: "rejected", code: "HTML_IMPORT_TOO_LARGE" });
  // Do not create a page for a document that can only contribute metadata or
  // executable code. This deliberately stays structural rather than attempting
  // pixel similarity: visible text and common rendered elements are enough.
  const sourceWithoutNonVisual = source
    .replace(/<script\b[^>]*>[\s\S]*?<\/script\s*>/gi, "")
    .replace(/<style\b[^>]*>[\s\S]*?<\/style\s*>/gi, "")
    .replace(/<[^>]+>/g, "")
    .replace(/&nbsp;|&#160;|\s+/gi, "");
  const hasRenderedElement = /<(?:img|svg|canvas|video|audio|input|button|select|textarea|iframe|object|embed|script)\b/i.test(source);
  if (!sourceWithoutNonVisual && !hasRenderedElement)
    return empty({ status: "rejected", code: "HTML_IMPORT_NOT_RENDERABLE" });
  let document: HtmlNode;
  try {
    document = parse(source) as unknown as HtmlNode;
  } catch {
    return empty({ status: "rejected", code: "HTML_IMPORT_INVALID" });
  }
  ...
  let numericViewport: { width: number; height: number } | undefined;
  let hasDeviceWidth = false;
  let hasViewportSignal = false;
  const visit = (node: HtmlNode, parent: string, index: number): void => {
    ...
    if (tag === "meta" && attrs.get("name")?.toLowerCase() === "viewport") {
      hasViewportSignal = true;
      const content = attrs.get("content") ?? "";
      hasDeviceWidth = /(?:^|,)\s*width\s*=\s*device-width\b/i.test(content);
      const width = /(?:^|,)\s*width\s*=\s*(\d+)/i.exec(content)?.[1];
      const height = /(?:^|,)\s*height\s*=\s*(\d+)/i.exec(content)?.[1];
      if (width && height)
        numericViewport = { width: Number(width), height: Number(height) };
    }
    if (tag && EMBEDS.has(tag))
    ...
    if (tag === "base")
      unsupportedCapabilities.push({ code: "base-url", path: itemPath });
    if (tag === "meta" && attrs.get("http-equiv")?.toLowerCase() === "refresh")
      unsupportedCapabilities.push({ code: "meta-refresh", path: itemPath });
    if (tag === "form") {
      hasForm = true;
      if (
        attrs.get("action")?.trim() ||
        attrs.get("method")?.trim().toLowerCase() === "post"
      )
        unsupportedCapabilities.push({
    ...
    if (tag === "style") {
      const cssData = addCssReferences(
        text(node),
        itemPath,
        resourceReferences,
        unsupportedCapabilities,
    ...
    if (tag === "script") {
      ...
      if (src) {
        ...
        if (classify(src) === "blob")
          unsupportedCapabilities.push({ code: "blob-script", path: itemPath });
      }
      if (executable) {
        signals.push({
        ...
        if (/\bwindow\.open\s*\(/.test(body))
          unsupportedCapabilities.push({ code: "popup", path: itemPath });
      } else if (!src && (DATA_SCRIPT_TYPES.test(type) || type !== ""))
        signals.push({
    ...
    if (tag)
      for (const [name, value] of attrs) {
        ...
        if (name === "download")
          unsupportedCapabilities.push({ code: "download", path: itemPath });
        ...
        for (const current of values) {
          ...
          if (classification === "data") {
            signals.push({
              code: "data-url-resource",
              path: itemPath,
              detail: name,
            });
            totalData += dataBytes(current);
            if (dataBytes(current) > HTML_IMPORT_MAX_DATA_URL_BYTES)
              unsupportedCapabilities.push({
                code: "data-url-too-large",
                path: itemPath,
                detail: `${dataBytes(current)}`,
              });
          }
        }
      }
    (node.childNodes ?? []).forEach((child, childIndex) =>
      visit(child, itemPath, childIndex),
    );
  };
  visit(document, "", 0);
  const interactive = signals.some((x) =>
    [
      "classic-script",
  ...
  if (hasForm && !interactive)
    unsupportedCapabilities.push({
      code: "form-submission",
      path: "/html/body/form",
    });
  if (totalData > HTML_IMPORT_MAX_TOTAL_DATA_URL_BYTES)
    unsupportedCapabilities.push({
      code: "data-url-too-large",
      path: "/",
      detail: `${totalData}`,
    });
  const restrictedWarnings = unsupportedCapabilities.map((capability) => ({
  ...
  const sorted = sortHtmlImportAnalysisCollections({
    ...
    unsupportedCapabilities,
    resourceReferences,
    warnings: restrictedWarnings,
  });
  const figmaViewport = detectFigmaExportViewport(source);
  // Resource availability affects compatibility, not execution. Only executable
  // document capabilities decide whether the isolated runtime is required.
  const requiresSandbox = interactive;
  ...
  return {
    analysis: {
      ...
      outcome,
      ...sorted,
      source: figmaViewport
        ? { kind: "figma-export", confirmationBypassEligible: true }
        : { kind: "unknown", confirmationBypassEligible: false },
      ...(title ? { detectedTitle: title } : {}),
      ...((numericViewport ?? figmaViewport)
        ? { detectedViewport: numericViewport ?? figmaViewport }
        : {}),
      presentation: recommendHtmlImportPresentation({
        figmaViewport,
        numericViewport,
...
export const analyzeHtmlImport = (source: string): HtmlImportAnalysis =>
  normalizeHtmlImport(source).analysis;

export interface HtmlImportBranchStageInput {
...
export interface HtmlImportBranchStage {
  analysis: HtmlImportAnalysis;
  normalizedHtml: string;
  stagingPath: string;
  commit(): void;
...
export function stageHtmlImportBranch(
  input: HtmlImportBranchStageInput,
): HtmlImportBranchStage {
  if (
    !input.workspacePath ||
    !fs.existsSync(input.workspacePath) ||
    !fs.statSync(input.workspacePath).isDirectory()
  )
  ...
  if (!input.schema || typeof input.schema !== "string")
    throw new HtmlImportError(
      "HTML_IMPORT_INVALID",
      "schema 必须是 JSON object",
    );
  try {
    const schema = JSON.parse(input.schema) as unknown;
  ...
  if (result.analysis.outcome.status !== "accepted")
    throw new HtmlImportError(result.analysis.outcome.code);
  ...
  try {
    ...
    const tree = fs.existsSync(treePath)
      ? (JSON.parse(fs.readFileSync(treePath, "utf8")) as {
          pages?: Array<Record<string, unknown>>;
          folders?: unknown[];
        })
      : { pages: [], folders: [] };
    if (
      (tree.pages ?? []).some((page) => page.id === input.pageId) ||
      fs.existsSync(path.join(input.workspacePath, "demos", input.pageId))
    )
      throw new HtmlImportError(
        "HTML_IMPORT_INVALID",
        `页面 id 已存在: ${input.pageId}`,
      );
    const pageDir = path.join(stagingPath, "demos", input.pageId);
    fs.mkdirSync(pageDir, { recursive: true });
    const sourceFile =
      runtimeType === "sandboxed-html" ? "sandbox.html" : "prototype.html";
    fs.writeFileSync(
      path.join(pageDir, sourceFile),
      result.normalizedHtml ?? "",
      "utf8",
    );
    if (runtimeType === "prototype-html-css")
      fs.writeFileSync(
        path.join(pageDir, "prototype.css"),
        input.css ?? "",
        "utf8",
      );
    const presentation =
      input.presentation ?? result.analysis.presentation.profile;
    fs.writeFileSync(
      path.join(pageDir, "config.schema.json"),
      applyPagePresentationToSchema(input.schema, presentation),
      "utf8",
    );
    if (runtimeType === "prototype-html-css") {
      fs.writeFileSync(
        ...
        JSON.stringify(
          {
            ...
            title: result.analysis.detectedTitle,
          },
          null,
          2,
        ) + "\n",
    ...
    tree.pages = [
      ...(tree.pages ?? []),
      { ...input.page, id: input.pageId, runtimeType },
    ];
  ...
  return {
    ...
    commit() {
      ...
      const backup = path.join(
        parent,
        `.html-import-backup-${path.basename(stagingPath).slice(".html-import-stage-".length)}`,
      );
      try {
        ...
        try {
          if (fs.existsSync(input.workspacePath) && fs.existsSync(backup))
            fs.rmSync(input.workspacePath, { recursive: true, force: true });
          if (fs.existsSync(backup) && !fs.existsSync(input.workspacePath))
            fs.renameSync(backup, input.workspacePath);
        } finally {
          fs.rmSync(stagingPath, { recursive: true, force: true });
          fs.rmSync(backup, { recursive: true, force: true });
          done = true;
