...
`@opencode-workbench/project-core` 是 Node-only 项目读写领域服务层，目标是让创作端 Web API、Project Admin MCP 和未来自动化入口复用同一套项目管理能力。
...
| `src/types.ts` | MCP 与 Web API 可复用的稳定业务类型 |
| `src/install-prompt.ts` | MCP 安装提示与配置生成 |
| `src/__tests__/` | 使用临时目录的单元测试 |

## 改动边界

- 本包只承载确定性的项目读写和校验逻辑，不依赖 Next.js、React、Fastify 或浏览器 API。
- Web API 与 MCP 都应调用本包，不要在调用方复制项目读写逻辑。
...
改动项目、模板、页面、文件夹、发布检查或 MCP 共享能力时，同步检查：
