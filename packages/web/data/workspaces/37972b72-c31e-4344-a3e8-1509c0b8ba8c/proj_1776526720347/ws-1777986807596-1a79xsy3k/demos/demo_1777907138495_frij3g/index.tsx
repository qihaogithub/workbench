...
const GradientCardsPage: React.FC<Props> = ({
  ...
  return (
    <div className="min-h-screen bg-gray-950 p-4 md:p-8 flex justify-center">
      {/* iPad Mini 尺寸容器 - 最大宽度 768px */}
      <div className="w-full max-w-[768px]">
        {/* 页面标题区域 */}
        <div className="text-center mb-8 md:mb-12">
          <h1 className="text-3xl md:text-5xl font-bold bg-gradient-to-r from-purple-400 via-pink-400 to-red-400 bg-clip-text text-transparent mb-3 md:mb-4">
            {title}
          </h1>
        ...
        <div className="mt-10 md:mt-16 text-center">
          <div className="inline-flex gap-2">
            {gradients.slice(0, 6).map((grad, i) => (
              <div
                key={i}
                className={`w-6 md:w-8 h-2 rounded-full bg-gradient-to-r ${grad}`}
              />
            ))}
          </div>
