// tmp/pdfs/import-challenge-prototype.ts
...
function isValidRouteKey(routeKey2) {
  return /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(routeKey2);
}
...
var ProjectAdminService = class {
  ...
  listProjects(actor2 = this.defaultActor()) {
    ...
    for (const entry of fs3.readdirSync(this.projectsDir, { withFileTypes: true })) {
      ...
      const project2 = this.readProject(entry.name);
      projects.push({
        id: entry.name,
        name: project2?.name ?? entry.name,
        category: normalizeProjectCategory(project2?.category),
        description: project2?.description,
        createdAt: project2?.createdAt ?? stats.birthtimeMs,
        updatedAt: project2?.updatedAt ?? stats.mtimeMs,
        thumbnail: project2?.thumbnail,
        demoCount: project2?.demoPages?.length ?? 0,
        demoPages: project2?.demoPages,
        publishedVersion: project2?.publishedVersion,
        publishedAt: project2?.publishedAt,
        locked: this.isProjectLocked(entry.name)
      });
    }
    return ok(projects.sort((a, b) => b.updatedAt - a.updatedAt));
  }
  getProject(projectId, actor2 = this.defaultActor()) {
    const access = this.requireProjectAccess(projectId, actor2);
    if (!access.ok)
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u65E0\u6743\u8BBF\u95EE\u8BE5\u9879\u76EE");
    const project2 = this.readProject(projectId);
    if (!project2)
    ...
    return ok({
      ...
      versions: [...project2.versions].reverse(),
      projectConfigSchema: this.readProjectConfig(workspacePath) ?? void 0,
      locked: this.isProjectLocked(projectId)
    });
  }
  exportProjectPackage(projectId, actor2 = this.defaultActor()) {
    const detail2 = this.getProject(projectId, actor2);
    if (!detail2.ok || !detail2.data) {
      return fail(detail2.error?.code ?? "PROJECT_NOT_FOUND", detail2.error?.message ?? "\u9879\u76EE\u4E0D\u5B58\u5728");
    }
    const workspacePath = this.projectWorkspacePath(projectId);
    const pages2 = [];
    for (const page of detail2.data.pages) {
    ...
    const baseVersion = detail2.data.versions[0]?.versionId ?? "v0";
    return ok({
      project: detail2.data.project,
      pages: pages2,
      folders: detail2.data.folders,
      versions: detail2.data.versions,
      projectConfigSchema: detail2.data.projectConfigSchema,
  ...
  createProject(input, actor2 = this.defaultActor()) {
    ...
    this.writeProject(projectId, project2);
    const stats = fs3.statSync(projectPath);
    const result2 = {
      id: projectId,
      name,
      ...
      demoCount: project2.demoPages.length,
      demoPages: project2.demoPages
    };
  ...
  updateProject(input, actor2 = this.defaultActor()) {
    ...
    if (!access.ok)
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u65E0\u6743\u8BBF\u95EE\u8BE5\u9879\u76EE");
    const project2 = this.readProject(input.projectId);
    if (!project2)
      return fail("PROJECT_NOT_FOUND", "\u9879\u76EE\u4E0D\u5B58\u5728");
    const diff2 = { updated: [] };
    const next = { ...project2 };
    ...
    if (!input.dryRun)
      this.writeProject(project2.id, next);
  ...
  duplicateProject(projectId, name, category, actor2 = this.defaultActor()) {
    const access = this.requireProjectAccess(projectId, actor2);
    if (!access.ok)
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u65E0\u6743\u8BBF\u95EE\u8BE5\u9879\u76EE");
  ...
  deleteProjectPreview(projectId, actor2 = this.defaultActor()) {
    const access = this.requireProjectAccess(projectId, actor2);
    if (!access.ok)
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u65E0\u6743\u8BBF\u95EE\u8BE5\u9879\u76EE");
    const project2 = this.readProject(projectId);
    ...
    const plan = this.createPlan("project_delete", projectId, [
      `\u5220\u9664\u9879\u76EE ${project2.name}`,
      `\u5220\u9664 ${project2.demoPages.length} \u4E2A\u9875\u9762`,
      "\u5220\u9664\u9879\u76EE\u5DE5\u4F5C\u7A7A\u95F4\u548C\u5143\u6570\u636E"
    ]);
    return ok(plan, {
  ...
  setProjectCover(projectId, thumbnail, actor2 = this.defaultActor()) {
    return this.updateProject({ projectId, description: void 0 }, actor2).ok ? this.patchProjectCover(projectId, thumbnail, actor2) : fail("PROJECT_NOT_FOUND", "\u9879\u76EE\u4E0D\u5B58\u5728");
  }
  ...
  createTemplateFromProject(projectId, input, actor2 = this.defaultActor()) {
    if (actor2.role === "readonly")
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u6CA1\u6709\u5199\u6743\u9650");
    const access = this.requireProjectAccess(projectId, actor2);
    if (!access.ok)
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u65E0\u6743\u8BBF\u95EE\u8BE5\u9879\u76EE");
    if (this.isProjectLocked(projectId) && actor2.role !== "admin") {
  ...
  beginEdit(projectId, actor2 = this.defaultActor()) {
    if (actor2.role === "readonly")
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u6CA1\u6709\u5199\u6743\u9650");
    const access = this.requireProjectAccess(projectId, actor2);
    if (!access.ok)
    ...
    const workspacePath = path3.join(this.workspacesDir, actor2.id, projectId, workspaceId);
    ...
    const transaction = {
      ...
      workspaceId,
      workspacePath,
      baseVersion: project2.versions.at(-1)?.versionId ?? "v0",
      actor: actor2,
      createdAt: Date.now(),
  ...
  commitEdit(editId, note, actor2 = this.defaultActor()) {
    ...
    const project2 = this.readProject(transaction.projectId);
    if (!project2)
      return fail("PROJECT_NOT_FOUND", "\u9879\u76EE\u4E0D\u5B58\u5728");
    const currentVersion = project2.versions.at(-1)?.versionId ?? "v0";
    ...
    const version = this.createProjectVersion(project2, projectWorkspace, actor2.name, editId, note, "named_version");
    ...
    const updatedProject = {
      ...
      demoPages: sortPages(tree.pages),
      demoFolders: tree.folders,
      versions: this.compactProjectVersions([...project2.versions, version]),
      updatedAt: Date.now()
    };
    this.writeProject(project2.id, updatedProject);
  ...
  pageVersionList(projectId, pageId, actor2 = this.defaultActor()) {
    const access = this.requireProjectAccess(projectId, actor2);
    if (!access.ok)
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u65E0\u6743\u8BBF\u95EE\u8BE5\u9879\u76EE");
    const project2 = this.readProject(projectId);
    if (!project2)
      return fail("PROJECT_NOT_FOUND", "\u9879\u76EE\u4E0D\u5B58\u5728");
    const versions = [...project2.pageVersions?.[pageId] ?? []].reverse();
    return ok({
      projectId,
      pageId,
  ...
  pageVersionGet(projectId, pageId, versionId, actor2 = this.defaultActor()) {
    const access = this.requireProjectAccess(projectId, actor2);
    if (!access.ok)
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u65E0\u6743\u8BBF\u95EE\u8BE5\u9879\u76EE");
    const project2 = this.readProject(projectId);
    if (!project2)
      return fail("PROJECT_NOT_FOUND", "\u9879\u76EE\u4E0D\u5B58\u5728");
    const version = project2.pageVersions?.[pageId]?.find((item) => item.versionId === versionId);
    if (!version)
      return fail("VERSION_NOT_FOUND", `\u9875\u9762\u7248\u672C ${versionId} \u4E0D\u5B58\u5728`);
    const files = this.readPageVersionFiles(project2, pageId, versionId);
    if (!files)
  ...
  createPageVersion(input, actor2 = this.defaultActor()) {
    ...
    const versionId = this.generateVersionId(project2);
    ...
    const existingVersions = project2.pageVersions?.[input.pageId] ?? [];
  ...
  restorePageVersion(projectId, pageId, versionId, actor2 = this.defaultActor()) {
    if (actor2.role === "readonly")
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u6CA1\u6709\u5199\u6743\u9650");
    const access = this.requireProjectAccess(projectId, actor2);
    if (!access.ok)
    ...
    if (!page)
      return fail("DEMO_PAGE_NOT_FOUND", "\u9875\u9762\u4E0D\u5B58\u5728");
    const targetVersion = project2.pageVersions?.[pageId]?.find((version2) => version2.versionId === versionId);
    if (!targetVersion)
      return fail("VERSION_NOT_FOUND", `\u9875\u9762\u7248\u672C ${versionId} \u4E0D\u5B58\u5728`);
    const files = this.readPageVersionFiles(project2, pageId, versionId);
    if (!files)
  ...
  publishCheck(projectId, actor2 = this.defaultActor()) {
    const detail2 = this.getProject(projectId, actor2);
    if (!detail2.ok || !detail2.data)
      return fail("PROJECT_NOT_FOUND", "\u9879\u76EE\u4E0D\u5B58\u5728");
    const validation2 = this.validateWorkspace(this.projectWorkspacePath(projectId));
    const issues = [...validation2.issues];
    if (detail2.data.pages.length === 0) {
      issues.push({ code: "NO_CONTENT_TO_PUBLISH", message: "\u9879\u76EE\u6CA1\u6709\u53EF\u53D1\u5E03\u9875\u9762", severity: "blocking" });
    }
    const result2 = { ok: issues.every((issue) => issue.severity !== "blocking"), issues };
    return ok(result2, { validation: result2 });
  }
  viewerBaseUrl() {
    return getViewerBaseUrl();
  }
  buildPublishStatus(projectId, input) {
    const project2 = this.readProject(projectId);
    const pages2 = project2?.demoPages ?? [];
    const artifactPath = input.artifactPath ?? path3.join(this.publishedDir, projectId);
  ...
  publishProject(projectId, actor2 = this.defaultActor()) {
    if (actor2.role === "readonly")
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u6CA1\u6709\u53D1\u5E03\u6743\u9650");
    const check = this.publishCheck(projectId, actor2);
    ...
    const versionedProject = {
      ...project2,
      versions: this.compactProjectVersions([...project2.versions, snapshot]),
      updatedAt: Date.now()
    };
    ...
    const status = this.buildPublishStatus(projectId, {
      ...
      artifactPath: path3.join(this.publishedDir, projectId)
    });
  ...
  publishStatus(projectId, actor2 = this.defaultActor()) {
    const access = this.requireProjectAccess(projectId, actor2);
    if (!access.ok)
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u65E0\u6743\u8BBF\u95EE\u8BE5\u9879\u76EE");
    const project2 = this.readProject(projectId);
    ...
    return ok(this.buildPublishStatus(projectId, {
      published: Boolean(project2.publishedVersion),
      publishedVersion: project2.publishedVersion,
      publishedAt: project2.publishedAt,
      artifactPath: fs3.existsSync(path3.join(this.publishedDir, projectId)) ? path3.join(this.publishedDir, projectId) : void 0
    }));
  }
  publishRollback(projectId, actor2 = this.defaultActor()) {
    const access = this.requireProjectAccess(projectId, actor2);
    if (!access.ok)
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u65E0\u6743\u8BBF\u95EE\u8BE5\u9879\u76EE");
    const project2 = this.readProject(projectId);
    if (!project2)
      return fail("PROJECT_NOT_FOUND", "\u9879\u76EE\u4E0D\u5B58\u5728");
    if (project2.versions.length < 2)
      return fail("VERSION_NOT_FOUND", "\u6CA1\u6709\u53EF\u56DE\u6EDA\u7684\u4E0A\u4E00\u7248\u672C");
    const previous = project2.versions.at(-2);
    const updated = {
      ...project2,
    ...
    this.writeProject(projectId, updated);
    return this.publishStatus(projectId, actor2);
  }
  auditList(projectId) {
    this.ensureDirs();
    const events = this.walkFiles(this.auditDir).filter((file) => file.endsWith(".json")).map((file) => readJsonFile(file)).filter((event) => Boolean(event)).filter((event) => !projectId || event.projectId === projectId).sort((a, b) => b.at - a.at);
    return ok(events);
  }
  auditGet(auditId) {
  ...
  lockProject(projectId, actor2 = this.defaultActor()) {
    if (actor2.role !== "admin")
  ...
  unlockProject(projectId, actor2 = this.defaultActor()) {
    if (actor2.role !== "admin")
      return fail("FORBIDDEN", "\u53EA\u6709\u7BA1\u7406\u5458\u53EF\u4EE5\u89E3\u9501\u9879\u76EE");
    const access = this.requireProjectAccess(projectId, actor2);
    if (!access.ok)
      return fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u65E0\u6743\u8BBF\u95EE\u8BE5\u9879\u76EE");
    fs3.rmSync(this.projectLockPath(projectId), { force: true });
    return ok({ unlocked: true, projectId });
  }
  patchProjectCover(projectId, thumbnail, actor2) {
    const project2 = this.readProject(projectId);
    if (!project2)
      return fail("PROJECT_NOT_FOUND", "\u9879\u76EE\u4E0D\u5B58\u5728");
    const updated = { ...project2, thumbnail, updatedAt: Date.now() };
    this.writeProject(projectId, updated);
  ...
  getProjectPath(projectId) {
    return path3.join(this.projectsDir, safeId(projectId, "project"));
  }
  getTemplatePath(templateId) {
    return path3.join(this.templatesDir, safeId(templateId, "template"));
  }
  projectWorkspacePath(projectId) {
    return path3.join(this.getProjectPath(projectId), "workspace");
  }
  readProject(projectId) {
    const parsed = readJsonFile(path3.join(this.getProjectPath(projectId), "project.json"));
    if (!parsed)
      return null;
    return {
  ...
  writeProject(projectId, project2) {
    writeJsonFile(path3.join(this.getProjectPath(projectId), "project.json"), project2);
  }
  ...
  createTemplateSnapshot(projectId, input) {
    const project2 = this.readProject(projectId);
    if (!project2)
    ...
    const template = {
      ...
      thumbnail: input.thumbnail ?? project2.thumbnail,
  ...
  readPageVersionFiles(project2, pageId, versionId) {
    const version = project2.pageVersions?.[pageId]?.find((item) => item.versionId === versionId);
  ...
  createProjectVersion(project2, workspacePath, actorName, editId, note, type = "named_version") {
    const versionId = this.generateVersionId(project2);
    const snapshotPath = path3.join(this.snapshotsDir, project2.id, versionId);
  ...
  generateVersionId(project2) {
    const pageVersions = Object.values(project2.pageVersions ?? {}).flat();
    const maxVersion = [...project2.versions, ...pageVersions].reduce((max, version) => {
  ...
  walkFiles(dir) {
    ...
    for (const entry of fs3.readdirSync(dir, { withFileTypes: true })) {
      ...
      const entryPath = path3.join(dir, entry.name);
      if (entry.isDirectory())
        result2.push(...this.walkFiles(entryPath));
      if (entry.isFile())
        result2.push(entryPath);
    }
    return result2;
  }
  ...
  validateSchemaPair(projectSchema, pageSchema2) {
    const issues = [];
    ...
    if (pageSchema2) {
      const parsed = this.parseSchema(pageSchema2, "config.schema.json", issues);
  ...
  projectLockPath(projectId) {
    return path3.join(this.internalDir, "locks", `${safeId(projectId, "project")}.json`);
  }
  isProjectLocked(projectId) {
    return fs3.existsSync(this.projectLockPath(projectId));
  }
  canAccessProject(projectId, actor2) {
    return !actor2.allowedProjectIds || actor2.allowedProjectIds.includes(projectId);
  }
  requireProjectAccess(projectId, actor2) {
    return this.canAccessProject(projectId, actor2) ? ok(true) : fail("FORBIDDEN", "\u5F53\u524D\u64CD\u4F5C\u8005\u65E0\u6743\u8BBF\u95EE\u8BE5\u9879\u76EE");
  }
  scanAiSessions(projectId) {
    ...
    for (const file of this.walkFiles(this.sessionsDir)) {
      ...
      if (projectId && parsedProjectId !== projectId)
        continue;
      sessions.push({
        sessionId,
  ...
  replaceReferences(workspacePath, oldPath, newPath) {
    ...
    for (const file of this.walkFiles(workspacePath)) {
      ...
      updated.push(path3.relative(workspacePath, file).split(path3.sep).join("/"));
    }
    return updated;
  }
...
function assertOk(label, result2) {
  if (!result2.ok || result2.data === void 0) {
    throw new Error(`${label} failed: ${JSON.stringify(result2, null, 2)}`);
  }
  return result2.data;
}
...
function pageCode(title, assetPath, width, height) {
...
function pageSchema(title, width, height) {
...
var project = assertOk("project create", service.createProject({
  name: "\u95EF\u5173\u6D3B\u52A8",
  category: "\u8425\u9500\u6D3B\u52A8",
  description: "\u7531 /Users/qh2/Downloads/\u95EF\u5173\u6D3B\u52A8.pdf \u539F\u578B\u56FE\u5BFC\u5165\uFF0C\u6BCF\u4E2A\u539F\u578B\u753B\u9762\u5BF9\u5E94\u4E00\u4E2A\u521B\u4F5C\u7AEF\u9875\u9762\u3002"
}, actor));
var edit = assertOk("edit begin", service.beginEdit(project.id, actor));
var pageResults = [];
var baseDir = path4.resolve("tmp/pdfs/challenge-pages");
for (const page of pages) {
  const imagePath = path4.join(baseDir, page.file);
  ...
  const upload = assertOk("asset upload", service.uploadAsset({
    ...
    filename: page.file,
    targetPath,
    dataBase64: fs4.readFileSync(imagePath).toString("base64"),
    mimeType: "image/png"
  }, actor));
...
var committed = assertOk("edit commit", service.commitEdit(edit.editId, "\u5BFC\u5165\u95EF\u5173\u6D3B\u52A8 PDF \u539F\u578B\u9875\u9762", actor));
var detail = assertOk("project get", service.getProject(project.id, actor));
var result = {
...
fs4.writeFileSync(
  path4.resolve("tmp/pdfs/challenge-import-result.json"),
