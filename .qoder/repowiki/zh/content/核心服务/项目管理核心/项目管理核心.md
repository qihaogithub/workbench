...
## 更新摘要
**变更内容**
- 新增集中式资源策略管理，通过 WorkspaceResourceRegistry 统一管理所有工作区资源的类型、路径规范、大小限制与内容校验
- 完善 TypeScript 定义，为工作区资源提供全面的类型系统支持
- 内置验证引擎，支持多种资源类型的结构化验证（JSON对象、工作区树、Sketch场景、二进制等）
- 增强资源清单生成机制，确保根哈希的一致性和内容敏感性
...
**最新更新**：增强了项目核心的资源管理能力，包括集中式资源策略管理、全面的工作区资源 TypeScript 定义和内置验证引擎。
...
**图表来源**
- [packages/project-core/src/service.ts:174-203](file://packages/project-core/src/service.ts#L174-L203)
...
**更新**：新增了集中的资源策略管理和内置验证引擎，提供更强大的资源管理能力。
...
**更新**：新增了集中式的资源策略管理和内置验证引擎。

- **资源发现**
  - 通过路径规则匹配资源类型（如页面代码、原型 HTML/CSS、Sketch 场景、项目 Schema、工作区树、知识文档、资产等）。
  - 支持 13 种不同的资源类型，每种都有明确的类型定义和约束。
- **依赖管理与约束**
  - 文本/二进制区分、最大字节限制（文本 2MB，二进制 20MB）。
  - JSON 对象或特定结构校验（如 workspace-tree、sketch-scene）。
  - 安全路径规范化，拒绝越界路径（包含 ..、空路径、NUL 字符）。
- **内置验证引擎**
  - 支持多种验证模式：text、json-object、workspace-tree、sketch-scene、binary。
  - 集成 Sketch 场景文档验证，确保手绘页面的数据结构完整性。
  - 工作区树结构验证，确保 pages 和 folders 数组的存在性。
- **版本控制**
  - 对每个资源计算内容哈希，生成资源清单与根哈希，用于一致性校验与增量同步。
  - 根哈希对所有资源内容的变化敏感，确保工作区状态的唯一性。
...
class ResourceValidationEngine {
...
WorkspaceResourceRegistry --> ResourceValidationEngine : "使用验证引擎"
```

**图表来源**
- [packages/project-core/src/workspace-resource-registry.ts:52-112](file://packages/project-core/src/workspace-resource-registry.ts#L52-L112)
- [packages/project-core/src/workspace-resource-registry.ts:114-135](file://packages/project-core/src/workspace-resource-registry.ts#L114-L135)
...
- [packages/project-core/src/__tests__/workspace-resource-registry.test.ts:1-114](file://packages/project-core/src/__tests__/workspace-resource-registry.test.ts#L1-114)

### 文件系统抽象层（虚拟文件系统、快照系统、差异计算）
- **虚拟文件系统**
  - 通过工作区资源注册表统一约束可写资源路径与内容格式，屏蔽具体存储实现。
  - 所有写入操作都必须经过资源注册表的策略验证。
- **快照系统**
...
**图表来源**
- [packages/project-core/src/types.ts:113-153](file://packages/project-core/src/types.ts#L113-153)
- [packages/project-core/src/config.ts:32-71](file://packages/project-core/src/config.ts#L32-L71)
- [packages/project-core/src/service.ts:174-203](file://packages/project-core/src/service.ts#L174-L203)
- [packages/author-site/src/lib/project-admin-service.ts:1-56](file://packages/author-site/src/lib/project-admin-service.ts#L1-L56)
- [packages/agent-service/src/routes/projects.ts:13-49](file://packages/agent-service/src/routes/projects.ts#L13-L49)
...
- **资源验证优化**：内置验证引擎采用懒加载策略，只在需要时执行复杂的结构验证。
...
- [data/audit/project-admin/2026-07-03/audit_1783068457408_7w9032.json:1-24](file://data/audit/project-admin/2026-07-03/audit_1783068457408_7w9032.json#L1-L24)
- [packages/agent-service/src/session/snapshot-service.ts:108-122](file://packages/agent-service/src/session/snapshot-service.ts#L108-L122)
- [packages/project-core/src/workspace-resource-registry.ts:73-88](file://packages/project-core/src/workspace-resource-registry.ts#L73-L88)

## 结论
项目管理核心通过 ProjectAdminService 提供了统一、可扩展且强一致性的项目管理能力，结合工作区资源注册表、快照与差异计算、Schema 驱动的校验、严格的权限与审计机制，形成了稳定可靠的工程基座。**最新的资源管理增强**进一步提升了系统的健壮性和可维护性，通过集中式的资源策略管理和内置验证引擎，确保了工作区数据的完整性和一致性。上层通过 Web 与 Agent 两种接入方式，既满足人类开发者也满足自动化代理的使用需求。
...
- **资源管理最佳实践**
  - 利用 WorkspaceResourceRegistry 进行资源策略验证。
  - 使用 createRootManifest 生成一致的资源清单。
  - 遵循内置验证引擎的规则，确保数据结构完整性。
