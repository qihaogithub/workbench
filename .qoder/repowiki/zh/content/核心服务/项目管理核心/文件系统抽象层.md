...
## 更新摘要
**所做更改**   
- 更新了工作空间管理模块结构，反映从fs-utils.ts到独立workspace-meta.ts的重构
- 添加了新的工作空间元数据管理章节
- 更新了导入路径和依赖关系图
- 保持了API表面不变性的说明
...
- author-site：工作区清单、页面元数据、自动保存调度器、混合格式解析器、工作空间元数据管理。
...
WASM["workspace-meta.ts<br/>工作空间元数据管理"]
...
**图表来源**
- [packages/author-site/src/lib/fs-utils.ts:2134-2141](file://packages/author-site/src/lib/fs-utils.ts#L2134-L2141)
...
- **新增** 工作空间元数据管理：独立的 workspace-meta.ts 模块，提供工作空间路径解析、元数据读写、版本标记等功能。
...
## 架构总览
下图展示了从编辑器到后端的核心交互链路：编辑触发自动保存调度器，调度器将脏资源批量提交；后端通过权限管理器校验路径与命令，使用快照服务进行差异比较与回滚；同时工作区清单与工作空间元数据管理保障元数据一致性与内容合法性。
...
- [packages/author-site/src/lib/workspace-autosave-scheduler.ts:1-69](file://packages/author-site/src/lib/workspace-autosave-scheduler.ts#L1-L69)
- [packages/author-site/src/app/demo/[id]/edit/page.tsx:4716-4746](file://packages/author-site/src/app/demo/[id]/edit/page.tsx#L4716-L4746)
- [packages/author-site/src/lib/workspace-meta.ts:14-44](file://packages/author-site/src/lib/workspace-meta.ts#L14-L44)
- [packages/agent-service/src/backends/managers/permission-manager.ts:1-200](file://packages/agent-service/src/backends/managers/permission-manager.ts#L1-L200)
- [packages/agent-service/src/session/snapshot-service.ts:1-342](file://packages/agent-service/src/session/snapshot-service.ts#L1-L342)
- [packages/author-site/src/lib/fs-utils.ts:558-697](file://packages/author-site/src/lib/fs-utils.ts#L558-L697)
...
### 工作空间元数据管理（workspace-meta）
**新增** 独立的工作空间元数据管理模块，提供以下核心功能：

- **路径解析**：`getWorkspacePath`、`findWorkspacePath`、`getWorkspaceDir` 函数提供工作空间路径的统一访问接口。
- **元数据读写**：`getWorkspaceMeta`、`writeWorkspaceMeta` 函数管理 `.workspace.json` 文件的读写操作。
- **版本标记**：`markWorkspaceBasedOnVersion` 函数用于标记工作空间的基准版本。
- **文件操作**：`getWorkspaceFiles`、`updateWorkspaceFiles` 函数提供工作空间文件的批量读写。
...
+getWorkspaceDir(userId, projectId) : string
...
**图表来源**
- [packages/author-site/src/lib/workspace-meta.ts:10-139](file://packages/author-site/src/lib/workspace-meta.ts#L10-L139)
...
## 依赖关系分析
- fs-utils 依赖共享类型与工作区清单读写，负责页面元数据与目录树一致性，并通过 re-export 保持向后兼容性。
- **新增** workspace-meta 作为独立模块，提供工作空间路径解析和元数据管理功能。
...
FSU --> WSMeta["workspace-meta.ts"]
WSMeta --> Paths["paths.ts"]
...
**图表来源**
- [packages/author-site/src/lib/fs-utils.ts:2134-2141](file://packages/author-site/src/lib/fs-utils.ts#L2134-L2141)
- [packages/author-site/src/lib/workspace-meta.ts:4](file://packages/author-site/src/lib/workspace-meta.ts#L4)
- [packages/agent-service/src/session/snapshot-service.ts:1-342](file://packages/agent-service/src/session/snapshot-service.ts#L1-L342)
- [packages/agent-service/src/backends/managers/permission-manager.ts:1-200](file://packages/agent-service/src/backends/managers/permission-manager.ts#L1-L200)
- [packages/agent-service/src/backends/pi-tools/permissions.ts:1-131](file://packages/agent-service/src/backends/pi-tools/permissions.ts#L1-L131)
- [packages/author-site/src/lib/workspace-autosave-scheduler.ts:1-69](file://packages/author-site/src/lib/workspace-autosave-scheduler.ts#L1-L69)
...
- 工作空间路径解析：`findWorkspacePath` 函数采用分层搜索策略，优先直接路径，然后递归搜索用户和项目目录，提高查找效率。
...
## 故障排查指南
- **新增** 工作空间路径问题：检查 `findWorkspacePath` 函数的搜索逻辑，确认 WORKSPACES_DIR 配置是否正确。
...
## 结论
本文件系统抽象层通过工作区清单、工作空间元数据管理、快照服务、权限管理与资源校验等核心组件，提供了稳健的 VFS 能力。其设计兼顾安全性（路径/命令白黑名单）、可靠性（快照与回滚）、性能（批处理与增量差异）与易用性（统一清单与解析器）。通过将工作空间管理函数重构到独立的 workspace-meta.ts 模块，代码结构更加清晰，职责分离更加明确，同时保持了 API 表面的向后兼容性。建议在业务逻辑中优先使用该抽象层提供的 API，遵循权限与资源校验规则，以获得一致且安全的文件操作体验。
...
### 工作空间元数据管理（workspace-meta）
**新增** 独立的工作空间元数据管理模块，提供以下 API：

- 关键能力
  - 路径解析：`getWorkspacePath`、`findWorkspacePath`、`getWorkspaceDir`
  - 元数据管理：`getWorkspaceMeta`、`writeWorkspaceMeta`
  - 版本标记：`markWorkspaceBasedOnVersion`
  - 文件操作：`getWorkspaceFiles`、`updateWorkspaceFiles`
- 建议
  - 使用 `findWorkspacePath` 而非直接拼接路径，确保跨平台兼容性。
  - 在操作工作空间前先检查 `workspaceExists`，避免不必要的文件操作。
  - 元数据更新时使用 `writeWorkspaceMeta` 确保原子性。
