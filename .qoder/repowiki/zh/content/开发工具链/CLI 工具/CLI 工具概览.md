...
- 命令分组
  ...
  - 创作端诊断：diagnostics
  - Workspace Authority：workspace-authority-* 系列命令
...
## 架构总览
CLI 作为轻量客户端，通过 HTTP 与 WebSocket 与 Agent Service 交互；部分本地诊断能力直接读取 data 目录下的 SQLite 与 JSONL 数据，支持远程拉取快照后离线分析。
...
- 只读查询
  - workspace-authority-status：查询 ready/revision/rootHash/queue/lease/prepared/drift 等健康指标
  - workspace-authority-preflight：将 health 转换为 passed/issues，供自动化门禁使用
- 受控修复（dry-run 默认，需 --apply 才写）
  - bootstrap：创建 Authority state
  - reconcile-adopt：接纳当前磁盘内容为新 revision
  - reconcile-restore：丢弃外部漂移，恢复最后 committed 内容
...
## 结论
该 CLI 工具围绕“快速验证、精准诊断、可自动化”的目标设计，覆盖从端到端消息交互到创作端事件溯源的全链路能力。通过统一的 --url 与 --json 全局选项，既满足交互式调试，也无缝融入 CI/CD 与运维流水线。Workspace Authority 系列命令为发布、导出、模板与部署前检查提供了强一致性的前置保障。
